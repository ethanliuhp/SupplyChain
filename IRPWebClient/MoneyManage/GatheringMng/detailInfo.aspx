<%@ Page Language="C#" AutoEventWireup="true" CodeFile="detailInfo.aspx.cs" Inherits="MoneyManage_GatheringMng_detailInfo" %>
...
<%--    <script src="../../JavaScript/jquery-1.10.2.min.js"></script>--%>
    ...
    //保存前进行验证
     ...
     {
        var bResult=false;
        var gatherType=document.getElementById("<%=cmbGatheringType.ClientID %>");
        if(gatherType.selectedIndex>0 &&gatherType.options[gatherType.selectedIndex].value){
            bResult=true;
        }
        else{
            showMessage(title+"失败:[收款类别]不能为空!");
            gatherType.focus();
        }
        return bResult;
     }
    ...
    function pageReadOnly(){
     ...
     function setReadyOnlyControl(){
         var arrReadyOnlyCtl=['<%=txtTheCustomerName.ClientID %>']; 
    ...
    function selectCustom(arrHT){
         var d=new SelectDialog("客户",
        function(data){
            if(data){
        ...
        <div id="divMaster" class="divMaster">
            ...
            <table cellpadding="0" cellspacing="0" class="MasterTable"  >
                <tr class="MasterTableTr">
                    ...
                    <td nowrap="nowrap" class="MasterTableTdText">
                        <asp:TextBox runat="server" ID="txtTheCustomerName"     Width="75%"></asp:TextBox>
                        <asp:HiddenField runat="server" ID="hdTheCustomerID" Value="" />
                        <input type="button" value="选择" <%=IsReadyOnly?"disabled='disabled' ":"" %>  onclick="selectCustom({'<%=hdTheCustomerID.ClientID %>':'cusrelid','<%=txtTheCustomerName.ClientID %>':'name'})" />
                    </td>
                    <td nowrap="nowrap" class="MasterTableTdLabel">收款日期:</td>
                    <td nowrap="nowrap" class="MasterTableTdText">
                ...
                <tr class="MasterTableTr">
                    <td nowrap="nowrap" class="MasterTableTdLabel">收款类别:</td>
                    <td nowrap="nowrap" class="MasterTableTdText">
                       <asp:DropDownList runat="server" ID="cmbGatheringType"></asp:DropDownList>
                    </td>
                    <td nowrap="nowrap" class="MasterTableTdLabel">备注:</td>
                    <td nowrap="nowrap" class="MasterTableTdText" colspan="3">
