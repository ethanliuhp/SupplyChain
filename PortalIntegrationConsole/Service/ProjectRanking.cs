...
namespace PortalIntegrationConsole.Service
{
    public class ProjectRanking
    {
        ...
        {
            string sql = "select id, ownerorgsyscode, createdate, projectname, handlepersonname projectmanager, projectlocationprovince, projectlocationcity, projectlocationdescript, projectinfostate," +
                          ...
                          " or t1.ownerorgsyscode like '%2F71AVYz91se5q3CJk37wx%' or t1.ownerorgsyscode like '%0kLVJlOoz3KPHWi7obsVpe%' or t1.ownerorgsyscode like '%3EfbkpOzv6MPvTDNE9jX6j%' or t1.ownerorgsyscode like '%1C12ZRo6fE89PqR8tYLnon%') ";
            ...
            var projectList = service.GetData(sql).Tables[0].Select().Select(a => new CurrentProjectInfo
            {
                ...
                ProjectInfoState = (EnumProjectInfoState)a["projectinfostate"]
            }).ToList();
...
union all 
select Translate(projectid USING NCHAR_CS) projectid, Translate(projectname USING NCHAR_CS) projectname, createdate, N'料具收料' indexname,count(*) count from thd_materialcollectionmaster where createdate>=to_date('{0}','yyyy-mm-dd') group by projectid, projectname, createdate
union all 
