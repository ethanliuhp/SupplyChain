...
namespace Application.Business.Erp.SupplyChain.ApprovalMng.AppSolutionMng.Service
{
    public interface IAppSrv : IBaseService
    {
        ...
        IList GetApprovingSets(string tName, string sysCode);

        IList GetApproveBillByType(string appSet, string billCode, string sysCode, DateTime bgDate, DateTime endDate);

        IList GetDefineStepsBySoulution(string souId);

        List<AppStepsInfo> GetAppStepsInfoByBill(ApproveBill bill);
    }
}
