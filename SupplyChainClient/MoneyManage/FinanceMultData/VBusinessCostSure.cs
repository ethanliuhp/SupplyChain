...
{
    public partial class VBusinessCostSure : TMasterDetailView
    {
        ...
        public VBusinessCostSure(AccountType accountType)
        {
            InitializeComponent();
            ...
            try
            {
                if (curBillMaster != null)
                {
                    ...
                    if (curBillMaster.Details != null && curBillMaster.Details.Count > 0)
                    {
                        ...
                        txtBusCostSure.Text = ShowMoney(oDetail.BusCostSure);
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                ...
                if (curBillMaster.Year == 0 || curBillMaster.Month == 0)
                {
                    MessageBox.Show("年份或月份选择存在问题,请重新选择！");
                    return false;
                }
                ...
                oDetail.BusCostSure = ClientUtil.ToDecimal(txtBusCostSure.Text);
                oDetail.Master = curBillMaster;
                oDetail.Money = oDetail.BusCostSure;
        ...
        public bool ValidView()
        {
            bool bReturn = false;
            decimal dValue = 0;
            if (decimal.TryParse(txtBusCostSure.Text, out dValue))
            {
                if (dValue > 0)
                {
                    bReturn = true;
                ...
                {
                    bReturn = false;
                    MessageBox.Show("[商务成本确认金额]必须大于0");
                }
            }
            else
            {
                bReturn = false;
                MessageBox.Show("请输入[商务成本确认金额]");
            }
            if (bReturn)
            {
                int iYear = ClientUtil.ToInt(this.cmbYear.SelectedItem);
                int iMonth = ClientUtil.ToInt(this.cmbMonth.SelectedItem);
                ObjectQuery oQuery = new ObjectQuery();
                oQuery.AddCriterion(Expression.Eq("Master.Year", iYear));
                oQuery.AddCriterion(Expression.Eq("Master.Month", iMonth));
                oQuery.AddCriterion(Expression.Gt("BusCostSure", 0M));
                oQuery.AddFetchMode("Master", FetchMode.Eager);
                //if (!string.IsNullOrEmpty(curBillMaster.Id))
                //{
                //    oQuery.AddCriterion(Expression.Sql("Master.Id",));
                //}
                IList lst = model.FinanceMultDataSrv.Query(typeof(FinanceMultDataDetail), oQuery);
                if (lst != null && lst.Count > 0)
                {
                    if (lst.Count == 1 && !string.IsNullOrEmpty(curBillMaster.Id) && string.Equals((lst[0] as FinanceMultDataDetail).Master.Id, curBillMaster.Id))
                    {
                        bReturn = true;
                    }
                    else
                    {
                        MessageBox.Show(string.Format("无法保存:[{0}年{1}月]的[项目商务成本确认]已经存在", iYear, iMonth));
                        bReturn = false;
                    }
                }
                else
