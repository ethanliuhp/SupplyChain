...
{
    public partial class VFundAssessment : TMasterDetailView
    {
        ...
        private void InitGrid()
        {
            ...
            SetGridLocked(gdInterestDetail, 13);
        }
        ...
        private void AfterYearMonthChanged()
        {
            ...
            gdInterestDetail.Cell(2, 1).Text = ym;
            gdInterestDetail.Cell(4, 2).Text = string.Format("{0}月", cmbMonth.Text);
        ...
        {
            if (currentMaster == null || currentMaster.Details.Count == 0 || currentMaster.AssessCashDetails.Count == 0)
            {
                return false;
            }
        ...
        private void InitEvents()
        {
            ...
            gdInterestDetail.CellChange += new Grid.CellChangeEventHandler(gdInterestDetail_CellChange);
        }

        private void gdInterestDetail_CellChange(object sender, Grid.CellChangeEventArgs e)
        {
            if (currentMaster == null || e.Row != 13)
            {
                return;
            }

            var val = 0m;
            decimal.TryParse(gdInterestDetail.Cell(e.Row, e.Col).Text.Trim(), out val);
            var iDetail = currentMaster.Details.OfType<FundInterestDetail>().FirstOrDefault();
            if (iDetail == null)
            {
                return;
            }

            iDetail.SettlementMoney = val * REPORT_UNIT;
            iDetail.ReceivableDebt = iDetail.SettlementMoney * currentMaster.GatheringRate - currentMaster.CurrentRealGet;
            DisplayInterestDetail(false);
        }
        ...
        private void pnlBody_SizeChanged(object sender, EventArgs e)
        {
            ...
            gdInterestDetail.Width = wid - 100;

            var pos = gdAssesscashDetail.Location;
            gdInterestDetail.Location = new Point(pos.X + gdAssesscashDetail.Width + 12, pos.Y);
        }
        ...
        private void btnGetData_Click(object sender, EventArgs e)
        {
            ...
            //currentMaster = mOperate.FinanceMultDataSrv.GetFundAssessmentData(currentMaster);
        ...
        private void DisplayInterestDetail(bool isShowSettlementMoney)
        {
            var rIndex = 3;
            gdInterestDetail.Cell(rIndex++, 2).Text = currentMaster.ProjectName;
            gdInterestDetail.Cell(rIndex++, 2).Text = string.Format("{0}月", currentMaster.CreateMonth);
            gdInterestDetail.Cell(rIndex++, 2).Text = currentMaster.ProjectState;
            gdInterestDetail.Cell(rIndex++, 2).Text = (currentMaster.CurrentSchemeTarget / REPORT_UNIT).ToString("N4");
            gdInterestDetail.Cell(rIndex++, 2).Text = (currentMaster.CurrentCashBalance / REPORT_UNIT).ToString("N4");

            var interestDetail = currentMaster.Details.FirstOrDefault() as FundInterestDetail;
            if (interestDetail != null)
            {
                gdInterestDetail.Cell(rIndex++, 2).Text = (interestDetail.CashBalanceInScheme / REPORT_UNIT).ToString("N4");
                gdInterestDetail.Cell(rIndex++, 2).Text = (interestDetail.CashBorrowInScheme / REPORT_UNIT).ToString("N4");
                gdInterestDetail.Cell(rIndex++, 2).Text = (interestDetail.CashBalanceOutScheme / REPORT_UNIT).ToString("N4");
                gdInterestDetail.Cell(rIndex++, 2).Text = (interestDetail.CashBorrowOutScheme / REPORT_UNIT).ToString("N4");
                gdInterestDetail.Cell(rIndex++, 2).Text = (interestDetail.InterestCost / REPORT_UNIT).ToString("N4");
                if (isShowSettlementMoney)
                {
                    var settlementCell = gdInterestDetail.Cell(rIndex, 2);
                    settlementCell.Text = (interestDetail.SettlementMoney/REPORT_UNIT).ToString("N4");
                    settlementCell.BackColor = editColor;
                    settlementCell.Locked = false;
                }
                rIndex++;
                gdInterestDetail.Cell(rIndex++, 2).Text = currentMaster.GatheringRate.ToString("P2");
                gdInterestDetail.Cell(rIndex++, 2).Text = (currentMaster.CurrentRealGet / REPORT_UNIT).ToString("N4");
                gdInterestDetail.Cell(rIndex++, 2).Text = (interestDetail.ReceivableDebt / REPORT_UNIT).ToString("N4");
                gdInterestDetail.Cell(rIndex, 2).Text = (interestDetail.CompleteInterestCost / REPORT_UNIT).ToString("N4");
            }
        }
