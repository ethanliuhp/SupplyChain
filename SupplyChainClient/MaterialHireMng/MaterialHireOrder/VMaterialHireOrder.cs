...
{
    public partial class VMaterialHireOrder : TMasterDetailView
    {
        CustomFlexGrid flexGrid1;
        private MMaterialHireMng model = new MMaterialHireMng();
        private MatHireOrderMaster curBillMaster;
        //private CurrentProjectInfo projectInfo = null;

        /// <summary>
        /// 当前单据
        /// </summary>
        public MatHireOrderMaster CurBillMaster
        {
            get { return curBillMaster; }
            set { curBillMaster = value; }
        }

        public VMaterialHireOrder()
        {
            InitializeComponent();
            InitEvent();
            InitData();
        ...
        public void InitData()
        {
            ...
            VBasicDataOptr.InitStationMatCostType(colCostType, true,null);
        ...
        private void InitEvent()
        {
            ...
            TenantSelector.TenantSelectorAfterEvent += (obj) => { if (obj.SelectedProject != null && curBillMaster!=null) { curBillMaster.ProjectId = obj.SelectedProject.Id; curBillMaster.ProjectName = obj.SelectedProject.Name; } };
        }
        ...
        void dgBasicCostDetail_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (!this.TenantSelector.Validate(true))
            {
                return;
            }
        ...
        void tsmiDel_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确定要删除当前选中的记录吗？", "删除记录", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                ...
                if (dr.Tag != null)
                {
                    curBillMaster.Details.Remove(dr.Tag as MatHireOrderDetail);
                }
            }
        }
        ...
        public void Start(string id)
        {
            try
            {
                ...
                {
                    curBillMaster = model.MaterialHireMngSvr.GetMaterialHireOrderById(id);
                    ModelToView();
        ...
        public override void RefreshControls(MainViewState state)
        {
            ...
            //永久锁定
            object[] os = new object[] { txtCode, txtCreatePerson, txtCreateDate1, txtHandlePerson };
            ObjectLock.Lock(os);
        ...
        public override bool NewView()
        {
            try
            {
                base.NewView();
                ClearView();
                this.curBillMaster = new MatHireOrderMaster();
        ...
        public override bool ModifyView()
        {
            ...
            {
                base.ModifyView();
                curBillMaster = model.MaterialHireMngSvr.GetMaterialHireOrderById(curBillMaster.Id);
                ModelToView();
                return true;
            }
        ...
        public override bool SaveView()
        {
            try
            {
                ...
                curBillMaster = model.MaterialHireMngSvr.SaveMaterialHireOrderMaster(curBillMaster);
                //插入日志
        ...
        public override bool DeleteView()
        {
            try
            {
                ...
                {
                    if (!model.MaterialHireMngSvr.DeleteByDao(curBillMaster)) return false;
                    ClearView();
                    return true;
                }
        ...
        private bool ValidView()
        {
            ...
            if (model.MaterialHireMngSvr.VerifyCurrSupplierOrder((txtSupply.Result[0] as SupplierRelationInfo), curBillMaster) == true)
            {
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                if (this.txtSupply.Result.Count > 0)
                {
                    ...
                    curBillMaster.SupplierID = curBillMaster.TheSupplierRelationInfo.SupplierInfo.Id;
                    curBillMaster.SupplierName = curBillMaster.TheSupplierRelationInfo.SupplierInfo.Name;
                }
                ...
                curBillMaster.BillCode = txtOldCode.Text;
                foreach (DataGridViewRow var in this.dgDetail.Rows)
                {
                    if (var.IsNewRow) break;
                    MatHireOrderDetail curBillDtl = new MatHireOrderDetail();
                    if (var.Tag != null)
                    {
                        curBillDtl = var.Tag as MatHireOrderDetail;
                   ...
                   curBillDtl.MaterialStuff = curBillDtl.MaterialResource == null ? null : curBillDtl.MaterialResource.Stuff;
                   curBillDtl.MaterialSysCode = curBillDtl.MaterialResource == null ? null : curBillDtl.MaterialResource.TheSyscode;
                   curBillDtl.MatStandardUnitName = ClientUtil.ToString(var.Cells[colUnit.Name].Value);
                    //curBillDtl.DiagramNumber=curBillDtl.MaterialResource ==null?null:curBillDtl.MaterialResource.
                    ...
                    if (var.Cells[colSetRule.Name].Tag != null)
                    {
                        ...
                        foreach (OrderDetailCostSetItem detail in curBillDtl.BasicDtlCostSets)
                        {
                            list_temp.Add(detail);
                        }
                        foreach (OrderDetailCostSetItem detail in list_temp)
                        {
                            curBillDtl.BasicDtlCostSets.Remove(detail);
                        }
                        //然后再增加
                        foreach (OrderDetailCostSetItem dtlCostSet in var.Cells[colSetRule.Name].Tag as IList)
                        {
                ...
                {
                    if (var.IsNewRow) break;
                    OrderMasterCostSetItem basicCostSet = new OrderMasterCostSetItem();
                    if (var.Tag != null)
                    {
                        basicCostSet = var.Tag as OrderMasterCostSetItem;
        ...
        private bool ModelToView()
        {
            try
            {
                ...
                this.TenantSelector.ProjectID = curBillMaster.ProjectId;
                txtOldCode.Text = curBillMaster.BillCode;
               ...
               // txtProject.Text = curBillMaster.ProjectName;
                ...
                foreach (MatHireOrderDetail var in curBillMaster.Details)
                {
                    ...
                    foreach (OrderDetailCostSetItem detial in var.BasicDtlCostSets)
                    {
                ...
                foreach (OrderMasterCostSetItem costSet in curBillMaster.BasiCostSets)
                {
        ...
        void dgDetail_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (this.dgDetail.Columns[e.ColumnIndex].Name.Equals(colMaterialCode.Name))
            {
                ...
                CommonMaterial materialSelector = new CommonMaterial();
                materialSelector.materialCatCode = CommonUtil.TurnStationMaterialMatCode;
                ...
                IList list = materialSelector.Result;
                string[] arrCode = dgDetail.Rows.OfType<DataGridViewRow>().Where(a => !a.IsNewRow).
                    Select(a=>ClientUtil.ToString( a.Cells[colMaterialCode.Name].Value)).ToArray();
                foreach (Application.Resource.MaterialResource.Domain.Material theMaterial in list)
                {
                    if (arrCode != null && arrCode.Length > 0 && arrCode.Contains(theMaterial.Code)) continue;
            ...
            if (this.dgDetail.Columns[e.ColumnIndex].Name.Equals(colSetRule.Name))
            {
                ...
                if (theCurrentRow.Cells[colMaterialCode.Name].Tag != null)
                {
                    ...
                    foreach (DataGridViewRow var in dgBasicCostDetail.Rows)
                    {
                        ...
                        OrderMasterCostSetItem costSet = new OrderMasterCostSetItem();
                    ...
                    bool IsUpdate= DataGridViewEditMode.EditOnEnter==dgDetail.EditMode;
                    VOrderMasterCostSetItem matCostSet = new VOrderMasterCostSetItem(theCurrentRow.Cells[colMaterialCode.Name].Tag as Material, list_CostSet, IsUpdate);
        ...
        private void FillFlex(MatHireOrderMaster billMaster)
        {
            ...
            {
                MatHireOrderDetail detail = (MatHireOrderDetail)billMaster.Details.ElementAt(i);
                //物资名称
