...
{
    public partial class VWebSitePlan : TMasterDetailView
    {
        private MProjectPlanningMng model = new MProjectPlanningMng();
        private SpecialityProposal curBillMaster;
        ...
        /// 当前表数据
        /// </summary>
        public SpecialityProposal CurBillMaster
        {
            ...
            if (curBillMaster.Id == null)
            {
                if (MessageBox.Show("当前业务对象还没保存，是否保存！", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        curBillMaster = model.ProjectPlanningSrv.SaveSpecialityProposal(curBillMaster);
        ...
        public void Start(string Id)
        {
            try
            {
                ...
                {
                    curBillMaster = model.ProjectPlanningSrv.GetSpecialityProposalById(Id);
                    ModelToView();
        ...
        public override void RefreshControls(MainViewState state)
        {
            ...
            //永久锁定
            object[] os = new object[] {txtCreatePerson, dtpCreateDate, txtProject, txtDcoState };
            ObjectLock.Lock(os);
        }
        ...
        public override bool NewView()
        {
            try
            {
                ...
                this.curBillMaster = new SpecialityProposal();
        ...
        public override bool ModifyView()
        {
            ...
            {
                base.ModifyView();
                curBillMaster = model.ProjectPlanningSrv.GetSpecialityProposalById(curBillMaster.Id);
                ModelToView();
                return true;
            }
        ...
        private bool ValidView()
        {
            ...
            {
                MessageBox.Show("专业策划名称不能为空！");
                return false;
            }
            if (ClientUtil.ToString(this.cmbEnginnerType.SelectedItem) == "")
            {
                MessageBox.Show("专业策划类型不能为空！");
                return false;
            }
            if (ClientUtil.ToString(this.cmbEvaluationWay.SelectedItem) == "")
            {
                MessageBox.Show("评审方式不能为空！");
                return false;
            }
            return true;
        }
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                curBillMaster.ProjectName = ClientUtil.ToString(txtProject.Text);
                ...
                curBillMaster.PlanningLevel = "专业策划";
                curBillMaster.SubmitDate = DateTime.Now;
                if (this.cmbEvaluationWay.SelectedItem.Equals("项目内部评审"))
                {
                    curBillMaster.EvaluationWay = 0;
                }
                if (this.cmbEvaluationWay.SelectedItem.Equals("公司评审"))
                {
                    curBillMaster.EvaluationWay = 1;
                }
                if (this.cmbEvaluationWay.SelectedItem.Equals("专家评审"))
                {
                    curBillMaster.EvaluationWay = 2;
                }
                curBillMaster.EnginnerType = ClientUtil.ToString(this.cmbEnginnerType.SelectedItem);
                return true;
            }
            catch (Exception e)
            {
        ...
        private bool ModelToView()
        {
            try
            {
                dtpCreateDate.Value = curBillMaster.CreateDate;
                txtRemark.Text = curBillMaster.Descript;
                this.txtCreatePerson.Tag = curBillMaster.CreatePerson;
                txtCreatePerson.Value = curBillMaster.CreatePersonName;
                this.txtProject.Text = ClientUtil.ToString(curBillMaster.ProjectName);
                this.txtProject.Tag = curBillMaster.ProjectId;
                txtEnginnerName.Text = ClientUtil.ToString(curBillMaster.EnginnerName);
                cmbEnginnerType.SelectedItem = ClientUtil.ToString(curBillMaster.EnginnerType);
                cmbEvaluationWay.SelectedItem = ClientUtil.ToString(curBillMaster.EvaluationWay);
                txtDcoState.Text = ClientUtil.GetDocStateName(curBillMaster.DocState);
                string evaluationway = ClientUtil.ToString(curBillMaster.EvaluationWay);
                if (evaluationway.Equals("0"))
                {
                    this.cmbEvaluationWay.SelectedItem = "项目内部评审";
                }
                if (evaluationway.Equals("1"))
                {
                    this.cmbEvaluationWay.SelectedItem = "公司评审";
                }
                if (evaluationway.Equals("2"))
                {
                    this.cmbEvaluationWay.SelectedItem = "专家评审";
                }
                FillDoc();  
                return true;
            }
