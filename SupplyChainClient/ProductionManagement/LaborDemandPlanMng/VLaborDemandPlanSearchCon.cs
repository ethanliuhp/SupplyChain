...
{
    public partial class VLaborDemandPlanSearchCon : BasicUserControl
    {
        private MLaborDemandPlanMng model = new MLaborDemandPlanMng();
        private VLaborDemandPlanSearchList searchList;
        public VLaborDemandPlanSearchCon(VLaborDemandPlanSearchList searchList)
        {
        ...
        void btnOK_Click(object sender, EventArgs e)
        {
            ...
            {
                oq.AddCriterion(Expression.Like("PlanName",this.txtPlanName.Text));
            }
            if (this.txtHandlePerson.Text != "")
            {
                oq.AddCriterion(Expression.Like("HandlePersonName",this.txtHandlePerson.Text));
            }
            CurrentProjectInfo projectInfo = StaticMethod.GetProjectInfo();
            oq.AddCriterion(Expression.Eq("CreatePerson", ConstObject.LoginPersonInfo));
            oq.AddCriterion(Expression.Eq("ProjectId", projectInfo.Id));
            oq.AddCriterion(Expression.Ge("ReportTime", this.dtpDateBegin.Value.Date));
            oq.AddCriterion(Expression.Lt("ReportTime", this.dtpDateEnd.Value.AddDays(1).Date));
            oq.AddOrder(new Order("Code", false));
            IList list = model.LaborDemandPlanSrv.GetLaborDemandPlan(oq);
