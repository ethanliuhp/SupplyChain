...
{
    public partial class VGWBSConfirm : TMasterDetailView
    {
        private MProductionMng model = new MProductionMng();

        private GWBSTaskConfirmMaster curBillMaster;

        Dictionary<string, List<WeekScheduleDetail>> dicDetail = new Dictionary<string, List<WeekScheduleDetail>>();
        ...
        void dgDetail_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            GWBSTaskConfirm confirmDetail = dgDetail.Rows[e.RowIndex].Tag as GWBSTaskConfirm;

            gridFinishedTask.Rows.Clear();
            if (confirmDetail != null && confirmDetail.WeekScheduleDetailGUID != null)
            {
                WeekScheduleDetail weekDtl = confirmDetail.WeekScheduleDetailGUID;

                //修改如果未加载前驱周计划明细则加载该数据
                try
                {
                    string SupplierName = weekDtl.SupplierName;
                }
                catch
                {
                    weekDtl = model.ProductionManagementSrv.GetObjectById(typeof(WeekScheduleDetail), weekDtl.Id) as WeekScheduleDetail;
                }

                int index = gridFinishedTask.Rows.Add();
                DataGridViewRow row = gridFinishedTask.Rows[index];

                row.Cells[colFinishTask.Name].Value = weekDtl.GWBSTreeName;
                row.Cells[colFinishTask.Name].ToolTipText = StaticMethod.GetCategorTreeFullPath(typeof(GWBSTree), weekDtl.GWBSTreeName, weekDtl.GWBSTreeSysCode);
                row.Cells[colFinishTask.Name].ToolTipText = StaticMethod.GetCategorTreeFullPath(typeof(GWBSTree), weekDtl.GWBSTreeName, weekDtl.GWBSTreeSysCode);

                row.Cells[colFinishAddupFigureprogress.Name].Value = weekDtl.TaskCompletedPercent;

                if (weekDtl.SubContractProject != null)
                    row.Cells[colFinishSubcontractProject.Name].Value = weekDtl.SupplierName;
        ...
        void tsmiCopy_Click(object sender, EventArgs e)
        {
            ...
            taskConfirmDetail.WeekScheduleDetailGUID = detail.WeekScheduleDetailGUID;

            for (int j = 0; j < dr.Cells.Count; j++)
            {
                if (dr.Cells[j].OwningColumn.Name != colSupplier.Name)
                {
                    dgDetail[j, rowIndex].Tag = dr.Cells[j].Tag;
                    dgDetail[j, rowIndex].Value = dr.Cells[j].Value;
                }
            }
            ...
            dgDetail_CellClick(dgDetail, new DataGridViewCellEventArgs(dgDetail.CurrentCell.ColumnIndex, dgDetail.CurrentCell.RowIndex));
        ...
        void btnProductSchedule_Click(object sender, EventArgs e)
        {
            ...
            List<WeekScheduleDetail> listWeekDtl = vss.SelectedWeekPlanDtl;
            ...
            foreach (WeekScheduleDetail WeekDtl in listWeekDtl)
            {
                dis.Add(Expression.And(Expression.Eq("ProduceConfirmFlag", 1), Expression.Eq("TheGWBS.Id", WeekDtl.GWBSTree.Id)));
            }
            oq.AddCriterion(dis);
            ...
            if (listSelTaskDtl.Count > 0)
            {
                ...
                //添加任务明细和周进度计划明细的关系
                //foreach (GWBSDetail taskDtl in listSelTaskDtl)
                //{
                //    var query = from w in listWeekDtl
                //                where w.GWBSTree.Id == taskDtl.TheGWBS.Id
                //                select w;

                //    if (query.Count() > 0)
                //    {

                //        if (!dicDetail.Keys.Contains(taskDtl.Id))
                //        {
                //            List<WeekScheduleDetail> listWD = new List<WeekScheduleDetail>();
                //            listWD.AddRange(query.ToList());
                //            dicDetail.Add(taskDtl.Id, listWD);
                //        }
                //        else
                //        {
                //            List<WeekScheduleDetail> listWD = dicDetail[taskDtl.Id];

                //            for (int i = 0; i < query.Count(); i++)
                //            {
                //                WeekScheduleDetail WeekDtl = query.ElementAt(i);

                //                for (int j = 0; j < listWD.Count; j++)
                //                {
                //                    WeekScheduleDetail dtl = listWD[j];
                //                    if (dtl.Id == WeekDtl.Id)
                //                    {
                //                        break;
                //                    }
            ...
            foreach (GWBSDetail detail in queryTaskDtl)
            {
                ...
                var queryWeekDtl = from w in listWeekDtl
                                   where w.GWBSTree.Id == detail.TheGWBS.Id
                                   select w;

                List<WeekScheduleDetail> listWeek = queryWeekDtl.ToList();

                if (listWeek.Count > 0)
                {
                    WeekScheduleDetail weekDtl = listWeek[0];

                    taskConfirmDetail.WeekScheduleDetailGUID = weekDtl;

                    if (!string.IsNullOrEmpty(weekDtl.TaskCheckState))
                        taskConfirmDetail.DailyCheckState = "2" + weekDtl.TaskCheckState.Substring(1);

                    if (weekDtl.SubContractProject != null)
                    {
                        taskConfirmDetail.TaskHandler = model.ProductionManagementSrv.GetObjectById(typeof(SubContractProject), listWeek[0].SubContractProject.Id) as SubContractProject;
                ...
                if (taskConfirmDetail.TaskHandler != null)
                {
                    ...
                    dgDetail[colSupplier.Name, rowIndex].Tag = taskConfirmDetail.TaskHandler;
                    dgDetail[colSupplier.Name, rowIndex].Value = taskConfirmDetail.TaskHandler.BearerOrgName;
                    dgDetail[colTheTeamQuantityConfirmed.Name, rowIndex].Value = model.ProductionManagementSrv.GetTheTeamQuantityConfirmed(projectInfo, detail, taskConfirmDetail.TaskHandler).ToString();
                }
            ...
            if (dgDetail.CurrentRow != null)
            {
                ...
                GWBSTaskConfirm confirmDetail = dgDetail.CurrentRow.Tag as GWBSTaskConfirm;
                ...
                if (confirmDetail != null && confirmDetail.WeekScheduleDetailGUID != null)
                {
                    ...
                    row.Cells[colFinishTask.Name].Value = confirmDetail.GWBSTreeName;
                    row.Cells[colFinishTask.Name].ToolTipText = StaticMethod.GetCategorTreeFullPath(typeof(GWBSTree), confirmDetail.GWBSTreeName, confirmDetail.GwbsSysCode);
                    ...
                    if (confirmDetail.TaskHandler != null)
                        row.Cells[colFinishSubcontractProject.Name].Value = confirmDetail.TaskHandlerName;

                    row.Tag = confirmDetail.WeekScheduleDetailGUID;
                }
            }
        }
        ...
        public override void RefreshControls(MainViewState state)
        {
            ...
            string[] lockCols = new string[] { colGWBSTree.Name, colGWBSDetail.Name, colSupplier.Name, colActualFinished.Name, colProjectType.Name, colProgress.Name, colUnit.Name, colTheTeamQuantityConfirmed.Name };
        ...
        public override bool NewView()
        {
            try
            {
                ...
                curBillMaster.OperOrgInfoName = ConstObject.TheOperationOrg.Name;
        ...
        public override bool ModifyView()
        {
            ...
            {
                MessageBox.Show("当前单据状态为【" + ClientUtil.GetDocStateName(curBillMaster.DocState) + "】,只能修改编辑状态的单据！");
                return false;
            }
        ...
        public override bool SubmitView()
        {
            ...
            //var query = from cf in curBillMaster.Details
            //            where StaticMethod.GetCheckStatePassStr(cf.DailyCheckState) == "未通过"
            //            select cf;

            //if (query.Count() > 0)//下属明细检查状态存在未通过的
            //{
            //    MessageBox.Show("下属确认单明细未全部通过检查，该单据不能提交审批！");
            //    return false;
            //}

            if (MessageBox.Show("提交后不能修改,确定要提交当前单据吗？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                try
                {
                    if (!ViewToModel()) return false;

                    curBillMaster.DocState = DocumentState.Valid;
        ...
        public override bool DeleteView()
        {
            try
            {
                ...
                if (curBillMaster.DocState == DocumentState.Edit)// || curBillMaster.DocState == DocumentState.Valid || curBillMaster.DocState == DocumentState.InAudit
                {
                    curBillMaster = model.ProductionManagementSrv.GetGWBSTaskConfirmMasterById(curBillMaster.Id);

                    if (!model.ProductionManagementSrv.DeleteGWBSTaskConfirmMaster(curBillMaster))
                        return false;
        ...
        private bool ValidView()
        {
            ...
            foreach (DataGridViewRow dr in dgDetail.Rows)
            {
                ...
                object quantityConfirmed = dr.Cells[colQuantityConfirmed.Name].Value;
                if (!StaticMethod.ValidDecimalCell(quantityConfirmed))
                {
            ...
            try
            {
                foreach (DataGridViewRow var in this.dgDetail.Rows)
                {
                    ...
                    if (curBillDtl.GWBSDetail == null)
                    {
                        ...
                        //curBillDtl.WeekScheduleDetail = dicDetail[detail.Id];
                    }
        ...
        private bool ModelToView()
        {
            try
            {
                ...
                foreach (GWBSTaskConfirm var in curBillMaster.Details)
                {
                    ...
                    dgDetail.Rows[i].Cells[colMaterialFeeSettlementFlag.Name].Value = var.MaterialFeeSettlementFlag.ToString();

                    this.dgDetail.Rows[i].Tag = var;
                }

                if (dgDetail.Rows.Count > 0 && dgDetail.CurrentCell != null)
                {
                    dgDetail_CellClick(dgDetail, new DataGridViewCellEventArgs(dgDetail.CurrentCell.ColumnIndex, dgDetail.CurrentCell.RowIndex));
                }
                return true;
        ...
        void dgDetail_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            else if (dgDetail[e.ColumnIndex, e.RowIndex].OwningColumn.Name == colQuantityConfirmed.Name)
            {
                cellValue = e.FormattedValue;
                if (cellValue == null || cellValue.ToString().Trim() == "")
                    return;
                ...
                if (decimal.Parse(cellValue.ToString()) < 0)
                {
                    MessageBox.Show("【本次确认工程量】要大于零。");
                    e.Cancel = true;
                }
            }
        ...
        private void dgDetail_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0 || e.ColumnIndex < 0)
                return;
            object cellValue = null;
            if (dgDetail[e.ColumnIndex, e.RowIndex].OwningColumn.Name == colSumQuantity.Name)//本次累计工程量
            {
                cellValue = dgDetail[e.ColumnIndex, e.RowIndex].Value;
                if (cellValue == null || cellValue.ToString().Trim() == "")
                    return;

                GWBSTaskConfirm bill = dgDetail.Rows[e.RowIndex].Tag as GWBSTaskConfirm;
                decimal addConfirmQny = decimal.Parse(cellValue.ToString());

                decimal quantityBefore = bill.QuantityBeforeConfirm;// ClientUtil.ToDecimal(dgDetail[colActualFinished.Name, e.RowIndex].Value);
                decimal currConfirmQny = addConfirmQny - quantityBefore;

                dgDetail[colQuantityConfirmed.Name, e.RowIndex].Value = currConfirmQny;

                if (bill.PlannedQuantity != 0)
                {
                    decimal currConfirmProgressValue = Decimal.Round((addConfirmQny / bill.PlannedQuantity) * 100, 2);
                    dgDetail.Rows[e.RowIndex].Cells[colSumCompletedPercent.Name].Value = currConfirmProgressValue;
                }
            }
            else if (dgDetail[e.ColumnIndex, e.RowIndex].OwningColumn.Name == colQuantityConfirmed.Name)//本次确认工程量
            {
                cellValue = dgDetail[e.ColumnIndex, e.RowIndex].Value;
                if (cellValue == null || cellValue.ToString().Trim() == "")
                    return;
                ...
                decimal quantityBefore = bill.QuantityBeforeConfirm;// ClientUtil.ToDecimal(dgDetail[colActualFinished.Name, e.RowIndex].Value);
                decimal currConfirmQny = decimal.Parse(cellValue.ToString());
                decimal addConfirmQny = currConfirmQny + quantityBefore;

                dgDetail[colSumQuantity.Name, e.RowIndex].Value = addConfirmQny;
            ...
            else if (dgDetail[e.ColumnIndex, e.RowIndex].OwningColumn.Name == colSumCompletedPercent.Name)//确认后累计形象进度
            {
                cellValue = dgDetail[e.ColumnIndex, e.RowIndex].Value;
                if (cellValue == null || cellValue.ToString().Trim() == "")
                    return;
                ...
                decimal addupProgress = decimal.Parse(cellValue.ToString()) / 100;
                decimal planQny = bill.PlannedQuantity;// ClientUtil.ToDecimal(dgDetail[colActualFinished.Name, e.RowIndex].Value);
                decimal addConfirmQny = addupProgress * planQny;
                decimal currConfirmQny = addConfirmQny - bill.QuantityBeforeConfirm;
