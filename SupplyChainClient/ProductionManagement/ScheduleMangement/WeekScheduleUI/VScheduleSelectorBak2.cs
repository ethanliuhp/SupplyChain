...
{
    public partial class VScheduleSelectorBak2 : TBasicDataView
    {
        ...
        public VScheduleSelectorBak2(EnumExecScheduleType frontPlanType)
        {
            InitializeComponent();
        ...
        private void InitForm()
        {
            ...
            if (cboSchedulePlanName.Items.Count > 0)
            {
                ...
                foreach (string name in cboSchedulePlanName.Items)
                {
                ...
                string scheduleName = cboSchedulePlanName.SelectedItem as string;
                ...
                oq.AddFetchMode("Detail", NHibernate.FetchMode.Eager);
                ...
                if (listMaster.Count > 0)
                {
                    for (int i = 0; i < listMaster.Count; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(item.ScheduleName))
                        {
                            ...
                            cbScheduleVersion.Items.Add(item);
                        }
                    }
                    ...
                    cbPlanVersion.DisplayMember = "ScheduleName";
                    cbScheduleVersion.DisplayMember = "ScheduleName";

                    cbPlanVersion.Text = CurBillMaster.ScheduleName;
        ...
        void cbScheduleVersion_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ProductionScheduleMaster master = cbScheduleVersion.SelectedItem as ProductionScheduleMaster;
            try
            {
                FlashScreen.Show("正在加载滚动进度计划......");
        ...
        private void AddPlanDtlInSelectGrid(ProductionScheduleDetail obj)
        {
        ...
        void btnAddInSelectGrid_Click(object sender, EventArgs e)
        {
            ...
            if (queryDtl.Count() > 0)
            {
                ...
                if (IsExistsPlanDetail(dtl) == false)
                {
                    ...
                    dtl = model.ProductionManagementSrv.ObjectQuery(typeof(ProductionScheduleDetail), oq)[0] as ProductionScheduleDetail;
        ...
        void btnAddLeftNodeInSelectGrid_Click(object sender, EventArgs e)
        {
            ...
            if (queryDtl.Count() > 0)
            {
                ...
                foreach (ProductionScheduleDetail dtlNode in queryDtl)
                {
                ...
                if (flag)
                {
                    ...
                    foreach (ProductionScheduleDetail dtlNode in listDtl)
                    {
        ...
        /// </summary>
        /// <returns></returns>
        private bool IsExistsPlanDetail(ProductionScheduleDetail dtl)
        {
            foreach (DataGridViewRow row in gridPlanDtlTree.Rows)
            {
                ProductionScheduleDetail item = row.Tag as ProductionScheduleDetail;
                if (item.Id == dtl.Id)
                    return true;
            }
        ...
        void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (tabControl1.SelectedTab == tabPageTable)//表方式
            {
                foreach (DataGridViewRow row in this.dgDetail.Rows)
                {
                    ...
                    {
                        ProductionScheduleDetail dtl = row.Tag as ProductionScheduleDetail;
                        ...
                        //    || (dtl.PlannedEndDate < DefaultBeginDate && dtl.AddupFigureProgress < 100))//“时间有交集”或者“滚动计划时间在前且任务未完工”

                        if (dtl.State == EnumScheduleDetailState.有效)
                            result.Add(dtl);
                ...
                if (result.Count > 0)
                {
                    ObjectQuery oq = new ObjectQuery();
                    Disjunction dis = new Disjunction();
                    foreach (ProductionScheduleDetail dtl in result)
                    {
                        dis.Add(Expression.Eq("Id", dtl.Id));
                    }
                    oq.AddCriterion(dis);
                    oq.AddFetchMode("GWBSTree", NHibernate.FetchMode.Eager);
                    IList listTempResult = model.ProductionManagementSrv.ObjectQuery(typeof(ProductionScheduleDetail), oq);

                    ProductionScheduleMaster master = (result[0] as ProductionScheduleDetail).Master;
                    foreach (ProductionScheduleDetail d in listTempResult)
                    {
                        d.Master = master;
                    }
        ...
        void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            ProductionScheduleMaster master = cbPlanVersion.SelectedItem as ProductionScheduleMaster;
            ...
            try
            {
                ...
                ProductionScheduleDetail d = new ProductionScheduleDetail();
