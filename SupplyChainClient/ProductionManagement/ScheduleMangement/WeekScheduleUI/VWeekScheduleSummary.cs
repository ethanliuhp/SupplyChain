...
{
    public partial class VWeekScheduleSummary : TMasterDetailView
    {
        ...
        void dgDetail_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
                ...
                )
            {
                ...
                for (int k = startColumnIndex; k <= endColumnIndex; k++)
                {
                    ...
                    if (k >= a && k <= b)
                    {
                        dgDetail.Rows[e.RowIndex].Cells[k].Value = true;
        ...
        void dgDetail_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    if (this.dgDetail.Columns[e.ColumnIndex].Name.Equals(colOBSService.Name))
                    {
                        ...
                        this.dgDetail.CurrentRow.Cells[colOBSService.Name].Tag = engineerMaster;
                        this.dgDetail.CurrentRow.Cells[colOBSService.Name].Value = engineerMaster.BearerOrgName;
                    }
                }
            }
            catch (Exception err)
        ...
        void btnWeekSchedule_Click(object sender, EventArgs e)
        {
            ...
            foreach (WeekScheduleDetail detail in list)
            {
                ...
                dgDetail[colWeekPlanName.Name, rowIndex].Value = detail.Master.PlanName;
                dgDetail[colOwner.Name, rowIndex].Value = detail.Master.HandlePersonName;
        ...
        public override void RefreshControls(MainViewState state)
        {
            ...
            string[] lockCols = new string[] { colWeekPlanName.Name, colOwner.Name, colGWBSTree.Name, colOBSService.Name };
        ...
        public override bool NewView()
        {
            try
            {
                ...
                curBillMaster.CreateDate = model.ProductionManagementSrv.GetServerTime();
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                ...
                curBillMaster.PlanName = txtName.Text;
                ...
                foreach (DataGridViewRow var in this.dgDetail.Rows)
                {
                    ...
                    curBillDtl.ForwardBillMasterName = ClientUtil.ToString(var.Cells[colWeekPlanName.Name].Value);
                    curBillDtl.ForwardBillMasterOwner = ClientUtil.ToString(var.Cells[colOwner.Name].Value);

                    //周计划
                    curBillDtl.Descript = ClientUtil.ToString(var.Cells[colDescript.Name].Value);
                    if (curBillDtl.GWBSTree == null && var.Cells[colGWBSTree.Name].Tag != null)
                    {
        ...
        private bool ModelToView()
        {
            try
            {
                ...
                txtName.Text = curBillMaster.PlanName;
                ...
                foreach (WeekScheduleDetail var in curBillMaster.Details)
                {
                    ...
                    if (!string.IsNullOrEmpty(var.ForwardBillMasterName))
                        dgDetail.Rows[i].Cells[colWeekPlanName.Name].Value = var.ForwardBillMasterName;
                    else if (!string.IsNullOrEmpty(var.ForwardBillDtlId))
                    {
                        WeekScheduleDetail frontDtl = model.ProductionManagementSrv.GetObjectById(typeof(WeekScheduleDetail), var.ForwardBillDtlId) as WeekScheduleDetail;
                        if (frontDtl != null && frontDtl.Master != null)
                            dgDetail.Rows[i].Cells[colWeekPlanName.Name].Value = frontDtl.Master.PlanName;
                    }

                    dgDetail.Rows[i].Cells[colOwner.Name].Value = var.ForwardBillMasterOwner;

                    dgDetail.Rows[i].Cells[colGWBSTree.Name].Tag = var.GWBSTree;
                    dgDetail.Rows[i].Cells[colGWBSTree.Name].Value = var.GWBSTreeName;

                    dgDetail.Rows[i].Cells[colGWBSTree.Name].ToolTipText = StaticMethod.GetCategorTreeFullPath(typeof(GWBSTree), var.GWBSTreeName, var.GWBSTreeSysCode);
        ...
        private void SetPlanView(DataGridViewRow dr, WeekScheduleDetail detail)
        {
            ...
            SetPlanView(dr, startColumnIndex, endColumnIndex);
        }

        private void SetPlanView(DataGridViewRow dr, int columnIndex, int endColumnIndex)
        {
            for (int i = columnIndex; i <= endColumnIndex; i++)
            {
                if (dgDetail.Columns[colMonday.Name].Index <= i && i <= dgDetail.Columns[colSunday.Name].Index)
        ...
        private void dgDetail_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //if (colName == colPlannedWrokload.Name)
            //{
        ...
        {
            if (LoadTempleteFile(@"施工生产周计划.flx") == false) return false;
        ...
        public override bool Export()
        {
            if (LoadTempleteFile(@"施工生产周计划.flx") == false) return false;
            FillFlex(curBillMaster);
            flexGrid1.ExportToExcel("施工生产周计划【" + curBillMaster.Code + "】", false, false, true);
            return true;
        }
        ...
        private void FillFlex(WeekScheduleMaster billMaster)
        {
            ...
            flexGrid1.Cell(3, 4).Text = "第" + ShowWeek(billMaster.CreateDate) + "周";
            ...
            flexGrid1.Cell(detailStartRowNumber + detailCount, 6).Text = billMaster.CreatePersonName;
        }
        private string ShowWeek(DateTime dt) 
        { 
