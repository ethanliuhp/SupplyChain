...
namespace Application.Business.Erp.SupplyChain.Client.ProductionManagement.InspectionRecordMng
{
    public partial class VGWBSDetailSelector : TBasicDataView
    {
        ...
        {
            if (dgDetail.Rows.Count <= 0 && this.dgDetail.SelectedRows.Count <= 0)
            {
                MessageBox.Show("请选择一条工程任务明细！");
                return;
            }
        ...
        private void ShowMasterList(IList masterList)
        {
            ...
            foreach (WeekScheduleDetail detail in masterList)
            {
                ...
                dgDetail[colDgMasterTeam.Name, rowIndex].Value = detail.SupplierName;
            }
