...
{
    public partial class VExpensesRowBillQuery : TBasicDataView
    {
        private MExpensesRowBillMng model = new MExpensesRowBillMng();

        public VExpensesRowBillQuery()
        {
            InitializeComponent();
            InitEvent();
            InitData();
        ...
        void dgDetail_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                ExpensesRowBill master = model.ExpensesRowBillSrv.GetExpensesRowBillByCode(dgvCell.Value.ToString());
                VExpensesRowBillMng vmro = new VExpensesRowBillMng();
        ...
        void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (rbBorrowDate.Checked)
            {
                if (StaticMethod.IsUseSQLServer())
                {
                    condition += " and IssueDate>='" + dtpDateBegin.Value.Date.ToShortDateString() + "' and IssueDate<'" + dtpDateEnd.Value.AddDays(1).Date.ToShortDateString() + "'";
                }
                else
                {
                    condition += " and IssueDate>=to_date('" + dtpDateBegin.Value.Date.ToShortDateString() + "','yyyy-mm-dd') and IssueDate<to_date('" + dtpDateEnd.Value.AddDays(1).Date.ToShortDateString() + "','yyyy-mm-dd')";
                }
            }
            ...
            {
                condition = condition + " and HandlePersonName='" + txtHandlePerson.Text + "'";
            }
            //制单人
            ...
            #endregion
            DataSet dataSet = model.ExpensesRowBillSrv.ExpensesRowBillQuery(condition);
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                string a = dataRow["IssueDate"].ToString();
                string[] aArray = a.Split(' ');
                string stra = aArray[0];
                dgDetail[colExpensesDate.Name, rowIndex].Value = stra;
                dgDetail[colExpensesPerson.Name, rowIndex].Value = dataRow["HandlePersonName"];
                ...
                dgDetail[colPaymentUnit.Name, rowIndex].Value = dataRow["PaymentUnit"];
                ...
                dgDetail[colDigest.Name,rowIndex].Value = dataRow["Digest"];
                dgDetail[colCollectionUnit.Name,rowIndex].Value = dataRow["CollectionUnit"];
