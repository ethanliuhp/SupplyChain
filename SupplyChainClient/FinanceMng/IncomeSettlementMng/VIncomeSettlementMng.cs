...
{
    public partial class VIncomeSettlementMng : TMasterDetailView
    {
        private MIncomeSettlementMng model = new MIncomeSettlementMng();
        private IncomeSettlementMaster curBillMaster;

        /// <summary>
        /// 当前单据
        /// </summary>
        public IncomeSettlementMaster CurBillMaster
        {
            get { return curBillMaster; }
            set { curBillMaster = value; }
        }

        public VIncomeSettlementMng()
        {
            InitializeComponent();
            InitEvent();
        }
        ...
        void tsmiDel_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确定要删除当前选中的记录吗？", "删除记录", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                ...
                if (dr.Tag != null)
                {
                    curBillMaster.Details.Remove(dr.Tag as IncomeSettlementDetail);
                }
            }
        }
        ...
        public void Start(string Id)
        {
            try
            {
                ...
                {
                    curBillMaster = model.IncomeSettlementSrv.GetIncomeSettlementMasterById(Id);
                    ModelToView();
        ...
        public override bool NewView()
        {
            try
            {
                ...
                this.curBillMaster = new IncomeSettlementMaster();
        ...
        public override bool ModifyView()
        {
            ...
            {
                base.ModifyView();
                curBillMaster = model.IncomeSettlementSrv.GetIncomeSettlementMasterById(curBillMaster.Id);
                ModelToView();
                return true;
            }
        ...
        public override bool SaveView()
        {
            try
            {
                ...
                curBillMaster = model.IncomeSettlementSrv.SaveIncomeSettlementMaster(curBillMaster);
        ...
        public override bool DeleteView()
        {
            try
            {
                ...
                {
                    if (!model.IncomeSettlementSrv.DeleteByDao(curBillMaster)) return false;
                    ClearView();
        ...
        private bool ValidView()
        {
            ...
            foreach (DataGridViewRow dr in dgDetail.Rows)
            {
                ...
                if (dr.Cells[colIncomeProject.Name].Value == null)
                {
                    MessageBox.Show("收益项目不允许为空！");
                    dgDetail.CurrentCell = dr.Cells[colIncomeProject.Name];
                    return false;
                }

                string strMoney = ClientUtil.ToString(dr.Cells[colIncomeCategories.Name].Value);
                if (strMoney == "")
                {
                    MessageBox.Show("收益类别不允许为空！");
                    return false;
                }

                string strMoney1 = ClientUtil.ToString(dr.Cells[colMoney.Name].Value);
                if (strMoney1 == "")
                {
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow var in this.dgDetail.Rows)
                {
                    ...
                    IncomeSettlementDetail curBillDtl = new IncomeSettlementDetail();
                    if (var.Tag != null)
                    {
                        curBillDtl = var.Tag as IncomeSettlementDetail;
                        if (curBillDtl.Id == null)
                        {
                            curBillMaster.Details.Remove(curBillDtl);
                        }
                    }
                    curBillDtl.IncomeProject = ClientUtil.ToString(var.Cells[colIncomeProject.Name].Value);
                    curBillDtl.Descript = ClientUtil.ToString(var.Cells[colDescript.Name].Value);
                    curBillDtl.IncomeCategories = ClientUtil.ToString(var.Cells[colIncomeCategories.Name].Value);
                    curBillDtl.Money = ClientUtil.ToDecimal(var.Cells[colMoney.Name].Value);
        ...
        private bool ModelToView()
        {
            try
            {
                ...
                this.txtCreateDate.Text = ClientUtil.ToString(curBillMaster.CreateDate.ToShortDateString());
                ...
                foreach (IncomeSettlementDetail var in curBillMaster.Details)
                {
                    int i = this.dgDetail.Rows.Add();
                    this.dgDetail[colIncomeProject.Name, i].Value = var.IncomeProject;
                    this.dgDetail[colIncomeCategories.Name, i].Value = var.IncomeCategories;
        ...
        //    //根据单价和数量计算金额  
        //    object money = dgDetail.Rows[e.RowIndex].Cells[colMoney.Name].Value;
        //    double summoney = 0;
        //    for (int i = 0; i < dgDetail.RowCount - 1; i++)
        //    {
        //        if (colName == colMoney.Name)
        //        {
        //            if (dgDetail.Rows[e.RowIndex].Cells[colMoney.Name].Value != null)
        //            {
        //                string temp_quantity = dgDetail.Rows[e.RowIndex].Cells[colMoney.Name].Value.ToString();
        //                validity = CommonMethod.VeryValid(temp_quantity);
        //                if (validity == false)
        //                    MessageBox.Show("请输入数字！");
        //                summoney += Convert.ToDouble(money);
        //            }
        //        }
        ...
        private void dgDetail_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            for (int i = 0; i <= dgDetail.RowCount - 1; i++)
            {
                if (colName == colMoney.Name)
                {
                    string Quantemp = ClientUtil.ToString(dgDetail.Rows[i].Cells[colMoney.Name].Value);
