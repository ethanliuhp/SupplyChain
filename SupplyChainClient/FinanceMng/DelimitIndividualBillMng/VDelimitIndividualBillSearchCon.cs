...
{
    public partial class VDelimitIndividualBillSearchCon : BasicUserControl
    {
        private MDelimitIndividualBillMng model = new MDelimitIndividualBillMng();
        private VDelimitIndividualBillSearchList searchList;

        public VDelimitIndividualBillSearchCon(VDelimitIndividualBillSearchList searchList)
        {
        ...
        void btnOK_Click(object sender, EventArgs e)
        {
            ...
            {
                oq.AddCriterion(Expression.Eq("Borrower", this.txtBorrowPerson.Text));
            }
            oq.AddCriterion(Expression.Ge("BorrowerDate", this.dtpDateBegin.Value.Date));
            oq.AddCriterion(Expression.Lt("BorrowerDate", this.dtpDateEnd.Value.AddDays(1).Date));
            oq.AddOrder(new Order("Code", false));
            IList list = model.DelimitIndividualBillSrv.GetDelimitIndividualBill(oq);
