...
{
    public partial class VDelimitIndividualBillMng : TMasterDetailView
    {
        private MDelimitIndividualBillMng model = new MDelimitIndividualBillMng();
        private DelimitIndividualBill curBillMaster;

        /// <summary>
        /// 当前单据
        /// </summary>
        public DelimitIndividualBill CurBillMaster
        {
            get { return curBillMaster; }
            set { curBillMaster = value; }
        }

        public VDelimitIndividualBillMng()
        {
            InitializeComponent();
            InitData();
        }
        ...
        {
            VBasicDataOptr.InitPayment(txtPayment, false);
        }
        ...
        public void Start(string Id)
        {
            try
            {
                ...
                {
                    curBillMaster = model.DelimitIndividualBillSrv.GetDelimitIndividualBillById(Id);
                    ModelToView();
        ...
        public override void RefreshState(MainViewState state)
        {
            ...
            switch (state)
            {
                ...
                case MainViewState.Modify:
                    ...
                    txtBorrowedPerson.Enabled = true;
                    txtBorrowUse.Enabled = true;
                    txtNodeNumber.Enabled = true;
                ...
                case MainViewState.Browser:
                    ...
                    txtBorrowedPerson.Enabled = false;
                    txtBorrowUse.Enabled = false;
                    txtNodeNumber.Enabled = false;
        ...
        public override bool NewView()
        {
            try
            {
                ...
                this.curBillMaster = new DelimitIndividualBill();
        ...
        public override bool ModifyView()
        {
            ...
            {
                base.ModifyView();
                curBillMaster = model.DelimitIndividualBillSrv.GetDelimitIndividualBillById(curBillMaster.Id);
                ModelToView();
                return true;
            }
        ...
        public override bool SaveView()
        {
            try
            {
                ...
                curBillMaster = model.DelimitIndividualBillSrv.SaveDelimitIndividualBill(curBillMaster);
        ...
        public override bool DeleteView()
        {
            try
            {
                ...
                {
                    if (!model.DelimitIndividualBillSrv.DeleteByDao(curBillMaster)) return false;
                    ClearView();
        ...
        private bool ValidView()
        {
            string validMessage = "";
            bool validity = true;
            string strName = ClientUtil.ToString(txtBorrowedPerson.Text);
            if (strName == "")
            {
                MessageBox.Show("借款人不能为空！");
                return false;                
            }
            ...
            else
            {
                ...
                if (validity == false)
                {
                    MessageBox.Show("借款金额要为数字！");
                    return false;
                }
            }
            string strNode = ClientUtil.ToString(txtNodeNumber.Text);
            if (strNode == "")
            {
                MessageBox.Show("票据号不能为空！");
                return false;
            }
            //string strUse = ClientUtil.ToString(txtBorrowUse.Text);
            //if (strUse == "")
            //{
            //    MessageBox.Show("借款用途不能为空！");
            //    return false;
            //}

            if (ClientUtil.ToDateTime(this.txtBorrowDate.Value.ToShortDateString()) > ClientUtil.ToDateTime(this.txtRepayDate.Value.ToShortDateString()))
            {
                MessageBox.Show("还款日期要大于借款日期！");
                return false;
            }
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                curBillMaster.Borrower = ClientUtil.ToString(txtBorrowedPerson.Text);//借款人
                curBillMaster.BorrowerDate = Convert.ToDateTime(txtBorrowDate.Text);//借款时间
                curBillMaster.BorrowerUse = ClientUtil.ToString(txtBorrowUse.Text);//借款用途
                curBillMaster.NodeNumber = ClientUtil.ToString(txtNodeNumber.Text);//票据号码
                curBillMaster.RepayDate = Convert.ToDateTime(txtRepayDate.Text);//还款日期
                curBillMaster.Payment = ClientUtil.ToString(txtPayment.SelectedItem);//支付方式
                curBillMaster.TotalMoney = Convert.ToDouble(txtTotalMoney.Text);//合计金额
                return true;
            }
            catch (Exception e)
            {
        ...
        private bool ModelToView()
        {
            try
            {
                ...
                txtRepayDate.Value = curBillMaster.RepayDate;
                txtPayment.SelectedValue = curBillMaster.Payment;
                txtNodeNumber.Text = curBillMaster.NodeNumber;
                txtPayment.SelectedItem = curBillMaster.Payment;
                txtBorrowUse.Text = curBillMaster.BorrowerUse;
                ...
                txtBorrowedPerson.Text = curBillMaster.Borrower;
                return true;
            }
            catch (Exception e)
            {
