...
{
    public partial class VDocumentDownloadByID : TBasicDataView
    {
        ...
        public VDocumentDownloadByID()
        {
            InitializeComponent();
            InitEvent();
        }
        /// <summary>
        /// 批量下载（传过来的是 工程对象关联文档集合）
        /// </summary>
        /// <param name="downList">下载列表(工程对象关联文档)</param>
        public VDocumentDownloadByID(IList downList)
        {
            InitializeComponent();
            //isIRPOrKB = irpOrKb;
            InitEvent();
        ...
        void ShowDownList(IList downList)
        {
            ...
            foreach (ProObjectRelaDocument doc in downList)
            {
                int rowIndex = this.dgvDownList.Rows.Add();
                dgvDownList[colDocumentName.Name, rowIndex].Value = doc.DocumentName;
                ...
                foreach (DataGridViewRow row in dgvDownList.Rows)
                {
                    ...
                    if (relaDoc.TheProjectCode != "KB")
                    {
                        ...
                        if (listResult[0].FileDataByte != null)
                        {
                            ...
                            {
                                dgvDownList[colDownloadState.Name, i].Value = "下载失败";
                                dgvDownList[colError.Name, i].Value = ExceptionUtil.ExceptionMessage(exc);
                            }
