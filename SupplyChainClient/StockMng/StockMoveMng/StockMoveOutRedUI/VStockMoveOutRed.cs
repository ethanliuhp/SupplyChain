...
{
    public partial class VStockMoveOutRed : TMasterDetailView
    {
        private MStockMng model = new MStockMng();
        private StockMoveOutRed curBillMaster;
        ...
        /// 当前单据
        /// </summary>
        public StockMoveOutRed CurBillMaster
        {
            get { return curBillMaster; }
            set { curBillMaster = value; }
        }

        public VStockMoveOutRed()
        {
            InitializeComponent();
            InitEvent();
            InitData();
        ...
        {
            VStockMoveOutSelector vsos = new VStockMoveOutSelector(execType);
            vsos.ShowDialog();
            IList list = vsos.Result;
            if (list == null || list.Count == 0) return;

            StockMoveOut tempMaster = list[0] as StockMoveOut;
            txtForward.Tag = tempMaster.Id;
            txtForward.Text = tempMaster.Code;

            txtMoveInProject.Tag = tempMaster.MoveInProjectId;
            txtMoveInProject.Text = tempMaster.MoveInProjectName;
            ...
            {
                StockMoveOutRedDtl dtl = dr.Tag as StockMoveOutRedDtl;
                if (dtl != null)
                {
            ...
            foreach (StockMoveOutDtl var in tempMaster.Details)
            {
                ...
                this.dgDetail[colMoney.Name, i].Value = (var.Quantity - var.RefQuantity) * var.Price;
                //调拨单价、金额
                dgDetail[colMovePrice.Name, i].Value = var.MovePrice;
                dgDetail[colMoveMoney.Name, i].Value = (var.Quantity - var.RefQuantity) * var.MovePrice;
        ...
        void tsmiDel_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确定要删除当前选中的记录吗？", "删除记录", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                ...
                if (dr.Tag != null)
                {
                    ...
                    movedDtlList.Add(dr.Tag as StockMoveOutRedDtl);
                }
            }
        }
        ...
        public void Start(string code)
        {
            try
            {
                ...
                {
                    curBillMaster = model.StockMoveSrv.GetStockMoveOutRedByCode(code, Enum.GetName(typeof(EnumStockExecType), execType), StaticMethod.GetProjectInfo().Id);
                    ModelToView();
        ...
        public void Start(string code,string id)
        {
            try
            {
                ...
                {
                    curBillMaster = model.StockMoveSrv.GetStockMoveOutRedById(id);
                    ModelToView();
                    RefreshState(MainViewState.Browser);
        ...
        public override void RefreshControls(MainViewState state)
        {
            ...
            //永久锁定
            object[] os = new object[] { txtCode, txtCreatePerson, txtCreateDate,txtHandlePerson,txtSumQuantity,txtProject,txtSumMoney,txtMoveInProject,txtForward,txtCat,txtForward};
            ObjectLock.Lock(os);

            string[] lockCols = new string[] { colMaterialStuff.Name, colMaterialCode.Name, colMaterialName.Name, colMaterialSpec.Name, colUnit.Name, colCanUseQty.Name, colPrice.Name, colMoney.Name, colProfessionalCategory.Name, colMovePrice.Name, colMoveMoney.Name };
        ...
        public override bool NewView()
        {
            try
            {
                ...
                this.curBillMaster = new StockMoveOutRed();
        ...
        public override bool ModifyView()
        {
            ...
            movedDtlList = new ArrayList();
            base.ModifyView();
            curBillMaster = model.StockMoveSrv.GetStockMoveOutRedById(curBillMaster.Id);
            ModelToView();
            return true;
        }
        ...
        public override bool SaveView()
        {
            try
            {
                ...
                {
                    curBillMaster = model.StockMoveSrv.SaveStockMoveOutRed(curBillMaster, movedDtlList);
                    //插入日志
                    StaticMethod.InsertLogData(curBillMaster.Id, "新增", curBillMaster.Code, ConstObject.LoginPersonInfo.Name, "调拨出库红单", "", curBillMaster.ProjectName);
                }
                else
                {
                    curBillMaster = model.StockMoveSrv.SaveStockMoveOutRed(curBillMaster, movedDtlList);
                    //插入日志
                    StaticMethod.InsertLogData(curBillMaster.Id, "修改", curBillMaster.Code, ConstObject.LoginPersonInfo.Name, "调拨出库红单", "", curBillMaster.ProjectName);
                }
                movedDtlList = new ArrayList();
        ...
        public override bool DeleteView()
        {
            try
            {
                ...
                model.StockMoveSrv.DeleteStockMoveOutRed(curBillMaster);
                //插入日志
                StaticMethod.InsertLogData(curBillMaster.Id, "删除", curBillMaster.Code, ConstObject.LoginPersonInfo.Name, "调拨出库红单", "", curBillMaster.ProjectName);
                ClearView();
                return true;
            }
            catch (Exception e)
        ...
        private bool ValidView()
        {
            ...
            foreach (DataGridViewRow dr in dgDetail.Rows)
            {
                ...
                StockMoveOutDtl forwardDetail = model.StockMoveSrv.GetStockMoveOutDtlById(forwardDtlId.ToString());
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                ...
                if (txtMoveInProject.Tag != null)
                {
                    curBillMaster.MoveInProjectId = txtMoveInProject.Tag.ToString();
                }
                curBillMaster.MoveInProjectName = txtMoveInProject.Text;
                ...
                foreach (DataGridViewRow var in this.dgDetail.Rows)
                {
                    ...
                    StockMoveOutRedDtl curBillDtl = new StockMoveOutRedDtl();
                    if (var.Tag != null)
                    {
                        curBillDtl = var.Tag as StockMoveOutRedDtl;
                    ...
                    curBillDtl.MoveMoney = -Math.Abs(ClientUtil.TransToDecimal(var.Cells[colMoveMoney.Name].Value));
        ...
        private bool ModelToView()
        {
            try
            {
                ...
                txtMoveInProject.Tag = curBillMaster.MoveInProjectId;
                txtMoveInProject.Text = curBillMaster.MoveInProjectName;
               ...
               // txtCreateDate.Text = curBillMaster.CreateDate.ToShortDateString();
                ...
                this.dgDetail.Rows.Clear();
                foreach (StockMoveOutRedDtl var in curBillMaster.Details)
                {
                    int i = this.dgDetail.Rows.Add();
                    ...
                    //调拨单价、金额
                    dgDetail[colMovePrice.Name, i].Value = var.MovePrice;
                    dgDetail[colMoveMoney.Name, i].Value = Math.Abs(var.MoveMoney);
                    tempMoveMoney += Math.Abs(var.MoveMoney);
            ...
            if (colName == colQuantity.Name)
            {
                ...
                for (int i = 0; i <= dgDetail.RowCount-1; i++)
                {
                    ...
                    if (i == e.RowIndex)
                    {
                        ...
                        //调拨金额
                        decimal movePrice = ClientUtil.TransToDecimal(dgDetail.Rows[i].Cells[colMovePrice.Name].Value);
                        decimal tempMoveMoney = movePrice * decQty;
                        dgDetail.Rows[i].Cells[colMoveMoney.Name].Value = tempMoveMoney.ToString("##########.##");
                        sumMoney += tempMoveMoney;
        ...
        public override bool Preview()
        {
            ...
            {
                if (LoadTempleteFile(@"调拨出库单（红单）.flx") == false) return false;
                FillFlex(curBillMaster);
                flexGrid1.PrintPreview(true, true, true, 0, 0, 0, 0, 0);
            }
            else
            {
                if (LoadTempleteFile(@"调拨出库单（红单）_安装.flx") == false) return false;
        ...
        void btnPrintPage_Click(object sender, FlexCell.Grid.PrintPageEventArgs e)
        {
            if (e.Preview == false && e.PageNumber == 1)
            {
                ...
                //写打印日志
                StaticMethod.InsertLogData(curBillMaster.Id, "打印", curBillMaster.Code, ConstObject.LoginPersonInfo.Name, "调拨出库红单", "", curBillMaster.ProjectName);
            }
        }
        ...
        private void FillFlex(StockMoveOutRed billMaster)
        {
            ...
            flexGrid1.Cell(3, 10).Text = billMaster.MatCatName;
            flexGrid1.Cell(3, 10).WrapText = true;
            ...
            {
                StockMoveOutRedDtl detail = (StockMoveOutRedDtl)billMaster.Details.ElementAt(i);
                ...
                //总值
                decimal sumMoney = (detail.RefQuantity - detail.Quantity) * detail.MovePrice;
                ...
                if (i == detailCount - 1)
                {
                    ...
                    flexGrid1.Cell(detailStartRowNumber + i + 1, 9).Text = ClientUtil.ToString(decimal.Round(sumMoney, 2));
        ...
        private void FillFlex1(StockMoveOutRed billMaster)
        {
            ...
            flexGrid1.Cell(3, 8).Text = billMaster.CreateDate.ToShortDateString();
            flexGrid1.Cell(4, 2).Text = billMaster.MoveInProjectName;
            flexGrid1.Cell(4, 8).Text = billMaster.Code;
            ...
            for (int i = 0; i < detailCount; i++)
            {
                ...
                if (i == detailCount - 1)
                {
                    ...
                    flexGrid1.Cell(detailStartRowNumber + i + 1, 2).Text = ClientUtil.ToString(Moneybig);
                }
                flexGrid1.Row(detailStartRowNumber + i).AutoFit();
            }
