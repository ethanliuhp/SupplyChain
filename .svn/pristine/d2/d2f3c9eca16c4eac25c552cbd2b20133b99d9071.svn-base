...
{
    public partial class VFundAssessmentSearchList : SearchList
    {
        private CFundPlan cFundPlan;

        public VFundAssessmentSearchList(CFundPlan cPaymentMng)
        {
            InitializeComponent();
        ...
        void dgSearchResult_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (master != null)
            {
                if (master.Code.Contains("考核兑现"))
                {
                    cFundPlan.FindFundAssessmentCash(master.Code, master.Id);
                }
                if (master.Code.Contains("利息计算"))
                {
                    cFundPlan.FindFundInterestCompute(master.Code, master.Id);
                }
            }
        }
