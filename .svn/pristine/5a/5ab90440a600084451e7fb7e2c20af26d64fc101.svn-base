...
namespace Application.Business.Erp.SupplyChain.CostManagement.WBS.Service
{
    ...
    /// 工程任务类型服务
    /// </summary>
    public class ProjectTaskTypeSrv : BaseService, IProjectTaskTypeSrv
    {
        #region 注入服务
        private IDao dao;
        public IDao Dao
        ...
        /// <returns></returns>
        [TransManager]
        public bool DeleteProjectTaskTypeTree(ProjectTaskTypeTree cate)
        {
            try
            {
                ...
                //1.删除WBS节点及其子节点
                string sql = " delete from thd_projecttasktypetree t1 where t1.syscode like '" + cate.SysCode + "%'";
                ...
                {
                    sql = "select count(*) childCount from thd_projecttasktypetree t1 where t1.parentnodeid='" + cate.ParentNode.Id + "'";
                    command.CommandText = sql;
                    int childCount = Convert.ToInt32(command.ExecuteScalar());

                    //根节点=0，枝节点=1，叶节点=2
                    if (childCount == 0)
                        sql = "update thd_projecttasktypetree t1 set t1.categorynodetype=2 where t1.id='" + cate.ParentNode.Id + "'";
                    else
                        sql = "update thd_projecttasktypetree t1 set t1.categorynodetype=1 where t1.id='" + cate.ParentNode.Id + "'";
        ...
        public bool DeleteProjectTaskTypeTree(IList list)
        {
            ...
            {
                DeleteProjectTaskTypeTree(node);
            }
        ...
        public IList GetProjectTaskTypeByInstance()
        {
            ...
            ObjectQuery oq = new ObjectQuery();
            //oq.AddCriterion(Expression.Not(Expression.Eq("State", (int)DocumentState.Invalid)));
        ...
        {
            ObjectQuery oq = new ObjectQuery();
            oq.AddCriterion(Expression.Like("SysCode", sNodeSysCode, MatchMode.Start));
            Disjunction dis = new Disjunction();
            dis.Add(Expression.Eq("State", (int )DocumentState.Edit));
            dis.Add(Expression.Eq("State", (int)DocumentState.InAudit));
            dis.Add(Expression.Eq("State", (int)DocumentState.Valid));
            dis.Add(Expression.Eq("State", (int)DocumentState.Invalid));
            oq.AddCriterion(dis);

            IList list = dao.ObjectQuery(typeof(ProjectTaskTypeTree), oq);

            foreach (ProjectTaskTypeTree item in list)
            {
                item.State = (int)DocumentState.InExecute;
                //item.TaskStateTime = DateTime.Now;
            }
           dao.SaveOrUpdate(list );
            //var query = from g in list.OfType<ProjectTaskTypeTree>()
            //            where g.SysCode == sNodeSysCode
            //            select g;
        ...
        public IList InvalidTaskNodeAndChilds(string sNodeSysCode)
        {
            ...
            dis.Add(Expression.Eq("State", (int)DocumentState.InAudit));
            dis.Add(Expression.Eq("State", (int)DocumentState.Valid));
            dis.Add(Expression.Eq("State", (int)DocumentState.InExecute));
            dis.Add(Expression.Eq("State", (int)DocumentState.Freeze));
            dis.Add(Expression.Eq("State", (int)DocumentState.Suspend));
            oq.AddCriterion(dis);
            ...
            foreach (ProjectTaskTypeTree item in list)
            {
                item.State =(int ) DocumentState.Invalid;
               // item.TaskStateTime = DateTime.Now;
            }
            dao.SaveOrUpdate(list);
            //var query = from g in list.OfType<ProjectTaskTypeTree>()
        ...
        public bool UpdateProjectTaskTypeTreeOrderNO(List<ProjectTaskTypeTree> lst)
        {
            try
            {
                ...
                {
                    sql += string.Format(@"update thd_ProjectTaskTypeTree set orderno ={0} where id = '{1}';", item.OrderNo, item.Id);
                }
                sql += "END;";
