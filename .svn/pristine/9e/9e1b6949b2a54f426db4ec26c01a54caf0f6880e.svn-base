...
{
    public partial class VQuantitiesAffirmSearchList : SearchList
    {
        private CProjectTaskAccount control;

        public VQuantitiesAffirmSearchList(CProjectTaskAccount c)
        {
            InitializeComponent();
            this.control = c;
            InitDgSearch();
        ...
        private void dgSearchResult_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (bill != null)
            {
                if (bill.FrontConfirmBillType ==
                    Application.Business.Erp.SupplyChain.ProductionManagement.ScheduleMangement.Domain.
                        EnumConfirmBillType.计划工单)
                    control.Find(bill.Code, bill.Id);
                else
                    control.Find3(bill.Code, bill.Id);
            }
        }
