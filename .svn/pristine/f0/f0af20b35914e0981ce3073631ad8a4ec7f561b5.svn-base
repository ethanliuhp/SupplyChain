...
{
    public partial class VExpensesSingleMng : TMasterDetailView
    {
        private MExpensesSingleBillMng model = new MExpensesSingleBillMng();
        private ExpensesSingleBill curBillMaster;

        /// <summary>
        /// 当前单据
        /// </summary>
        public ExpensesSingleBill CurBillMaster
        {
            get { return curBillMaster; }
            set { curBillMaster = value; }
        }

        public VExpensesSingleMng()
        {
            InitializeComponent();
            VBasicDataOptr.InitExpensesCose(txtExpensesType, false);
        ...
        public void Start(string Id)
        {
            try
            {
                ...
                {
                    curBillMaster = model.ExpensesSingleBillSrv.GetExpensesSingleBillById(Id);
                    ModelToView();
        ...
        //金额大写的转化
        void txtExpensesMoney_tbTextChanged(object sender, EventArgs e)
        {
            string billAmount = txtExpensesMoney.Text;
            try
            {
        ...
        //金额大写的转化
        void txtCHJKMoney_tbTextChanged(object sender, EventArgs e)
        {
            string billAmount = txtCHJKMoney.Text;
            try
            {
                txtCHJKBig.Text = CurrencyComUtil.GetMoneyChinese(billAmount);
        ...
        public override void RefreshState(MainViewState state)
        {
            ...
            switch (state)
            {
                ...
                case MainViewState.Modify:
                    //txtCHJKBig.Enabled = true;
                    txtCHJKMoney.Enabled = true;
                    txtDigest.Enabled = true;
                    txtExpensesDate.Enabled = true;
                    txtExpensesMoney.Enabled = true;
                    txtExpensesPerson.Enabled = true;
                    txtExpensesType.Enabled = true;
                    cmsDg.Enabled = true;
                    break;
                case MainViewState.Initialize:
                case MainViewState.Browser:
                    //txtCHJKBig.Enabled = false;
                    txtCHJKMoney.Enabled = false;
                    txtDigest.Enabled = false;
                    txtExpensesDate.Enabled = false;
                    txtExpensesMoney.Enabled = false;
                    txtExpensesPerson.Enabled = false;
                    txtExpensesType.Enabled = false;
                    cmsDg.Enabled = false;
                    break;
                default:
                    break;
        ...
        public override bool NewView()
        {
            try
            {
                ...
                this.curBillMaster = new ExpensesSingleBill();
        ...
        public override bool ModifyView()
        {
            ...
            {
                base.ModifyView();
                curBillMaster = model.ExpensesSingleBillSrv.GetExpensesSingleBillById(curBillMaster.Id);
                ModelToView();
                return true;
            }
        ...
        public override bool SaveView()
        {
            try
            {
                ...
                curBillMaster = model.ExpensesSingleBillSrv.SaveExpensesSingleBill(curBillMaster);
        ...
        public override bool DeleteView()
        {
            try
            {
                ...
                {
                    if (!model.ExpensesSingleBillSrv.DeleteByDao(curBillMaster)) return false;
                    ClearView();
        ...
        private bool ValidView()
        {
            string validMessage = "";
            bool validity = true;
            string strName = ClientUtil.ToString(txtExpensesPerson.Text);
            if (strName == "")
            {
                MessageBox.Show("报销人不能为空！");
                return false;
            }
            string strMoney = ClientUtil.ToString(txtExpensesMoney.Text);
            if (strMoney == "")
            {
                MessageBox.Show("报销金额不能为空！");
                return false;
            }
            else
            {
                ...
                if (validity == false)
                {
                    MessageBox.Show("报销金额为数字！");
                    return false;
                }
            }

            string strMoney1 = ClientUtil.ToString(txtCHJKMoney.Text);
            if (strMoney1 == "")
            {
                MessageBox.Show("冲借款金额不能为空！");
                return false;
            }
            else
            {
                ...
                if (validity == false)
                {
                    MessageBox.Show("冲借款金额为数字！");
                    return false;
                }
            }

            if (Convert.ToDouble(this.txtCHJKMoney.Text) > Convert.ToDouble(this.txtExpensesMoney.Text))
            {
                MessageBox.Show("报销金额要大于冲借款金额！");
                return false;
            }
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                ...
                curBillMaster.ExpensesDate = Convert.ToDateTime(txtExpensesDate.Value);//报销日期
                curBillMaster.ExpensesPerson = ClientUtil.ToString(txtExpensesPerson.Text);//报销人
                curBillMaster.ExpensesType = ClientUtil.ToString(txtExpensesType.SelectedItem);//费用类型
                curBillMaster.Purpose = ClientUtil.ToString(txtDigest.Text);//费用说明
                curBillMaster.TotalMoney = Convert.ToDouble(txtExpensesMoney.Text);//报销金额
                curBillMaster.Money = Convert.ToDouble(txtCHJKMoney.Text);//冲借款金额
        ...
        private bool ModelToView()
        {
            try
            {
                ...
                this.txtCHJKMoney.Text = ClientUtil.ToString(curBillMaster.Money);
                this.txtDigest.Text = curBillMaster.Purpose;
                this.txtExpensesDate.Value = curBillMaster.ExpensesDate;
                this.txtExpensesMoney.Text = ClientUtil.ToString(curBillMaster.TotalMoney);
                this.txtExpensesPerson.Text = curBillMaster.ExpensesPerson;
                this.txtExpensesType.SelectedItem = curBillMaster.ExpensesType;
