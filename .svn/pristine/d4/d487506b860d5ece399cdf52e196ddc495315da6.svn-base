...
{
    public partial class VMaterialHireCollectionQuery : TBasicDataView
    {
        private MMaterialHireMng model = new MMaterialHireMng();
        public VMaterialHireCollectionQuery()
        {
            InitializeComponent();
            InitData();
            InitEvent();
        ...
        void dgMaster_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                MatHireCollectionMaster master = dgMaster.Rows[e.RowIndex].Tag as MatHireCollectionMaster;
                VMaterialHireCollection vMaterial = new VMaterialHireCollection(master.MatHireType);
                vMaterial.RefreshControls(VirtualMachine.Component.WinMVC.generic.MainViewState.Browser);
                vMaterial.Start(master.Id);
                vMaterial.StartPosition = FormStartPosition.CenterParent;
                vMaterial.ShowDialog();
        ...
        void dgDetail_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex > -1 && e.ColumnIndex > -1)
            {
                ...
                {
                    MatHireCollectionMaster oMaterialHireCollectionMaster = model.MaterialHireMngSvr.GetMaterialHireCollectionMasterById(billId);
                    VMaterialHireCollection vOrder = new VMaterialHireCollection(oMaterialHireCollectionMaster.MatHireType);
                    vOrder.RefreshControls(VirtualMachine.Component.WinMVC.generic.MainViewState.Browser);
                    vOrder.Start(billId);
                    vOrder.StartPosition = FormStartPosition.CenterParent;
                    vOrder.ShowDialog( );
        ...
        void dgDetail_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvCell.OwningColumn.Name == colCode.Name)
            {
                ...
                {
                    MatHireCollectionMaster master = model.MaterialHireMngSvr.GetMaterialHireCollectionMasterById(billId);//GetMaterialHireCollectionMasterByCode(ClientUtil.ToString(dgvCell.Value));
                    VMaterialHireCollection vMaterial = new VMaterialHireCollection(master.MatHireType);
                    vMaterial.RefreshControls(VirtualMachine.Component.WinMVC.generic.MainViewState.Browser); 
                    vMaterial.Start(master.Id);
                    vMaterial.StartPosition = FormStartPosition.CenterParent;
        ...
        void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                #endregion
                DataSet dataSet = model.MaterialHireMngSvr.MaterialHireCollectionMasterQuery(condition);
                this.dgDetail.Rows.Clear();
                EnumMatHireType matType;
                ...
                foreach (DataRow dataRow in dataTable.Rows)
                {
                    ...
                    //t2.materiallength,t2.materialtype
                    dgDetail[this.colBillCode.Name, rowIndex].Value = ClientUtil.ToString(dataRow["BillCode"]);
                    matType =  (EnumMatHireType) ClientUtil.ToInt(dataRow["mathiretype"]);
                    dgDetail[this.colDtlMatType.Name, rowIndex].Value = matType == EnumMatHireType.碗扣 ? ClientUtil.ToString(dataRow["materialtype"]) : "";
                    dgDetail[this.colDtlMatLen.Name, rowIndex].Value = matType == EnumMatHireType.钢管 ? ClientUtil.ToString(dataRow["materiallength"]) : "";
                }
                FlashScreen.Close();
        ...
        void btnSearchBill_Click(object sender, EventArgs e)
        {
          ...
          //  oq.AddCriterion(Expression.Eq("ProjectId", projectInfo.Id));
            ...
            IList list = model.MaterialHireMngSvr.ObjectQuery(typeof(MatHireCollectionMaster), oq);
            dgMaster.Rows.Clear();
        ...
        void ShowMasterList(IList list)
        {
            ...
            foreach (MatHireCollectionMaster master in list)
            {
                ...
                this.dgMaster[this.colQTMoneyMaster.Name, i].Value = master.SumExtMoney;
                this.dgMaster.Rows[i].Tag = master;
                this.dgMaster[colBillCodeMaster.Name, i].Value = master.BillCode;
                this.dgMaster[colProjectNameMaster.Name, i].Value = master.ProjectName;
            }
        ...
        {
            dgDetailBill.Rows.Clear();
            MatHireCollectionMaster master = dgMaster.CurrentRow.Tag as MatHireCollectionMaster;
            if (master == null) return;
            if (master.MatHireType == EnumMatHireType.普通料具)
            {
                colMatType.Visible = false;
            }
            else
            {
                colMatType.Visible = true;
                colMatType.HeaderText = master.MatHireType == EnumMatHireType.钢管 ? "长度" : "型号";
            }
            foreach (MatHireCollectionDetail dtl in master.Details)
            {
                ...
                this.dgDetailBill[this.colMatType.Name, i].Value = master.MatHireType == EnumMatHireType.碗扣 ? dtl.MaterialType : dtl.MaterialLength.ToString();
            }
        }
    }
}
