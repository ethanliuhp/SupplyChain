...
{
    public partial class VTimeDispatching : TMasterDetailView
    {
        ...
        private void InitEvent()
        {
            ...
            txtSporadicType.SelectedIndexChanged += new EventHandler(txtSporadicType_SelectedIndexChanged);
        }

        void txtSporadicType_SelectedIndexChanged(object sender, EventArgs e)
        {
            type = txtSporadicType.SelectedItem.ToString();

            if ("计时派工" == type)
            {
                var unit = GetUnitByName("工日");
                for (int i = 0; i < dgDetail.Rows.Count - 1; i++)
                {
                    dgDetail.Rows[i].Cells[colQuantityUnit.Name].Value = unit.Name;
                    dgDetail.Rows[i].Cells[colQuantityUnit.Name].Tag = unit;
                }
            }
        }
        ...
        void cmsDg_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            //else if (e.ClickedItem.Text.Trim() == "新增行")
            //{
            //    cmsDg.Hide();
            //    dgDetail.Rows.Add();
            //}
            else if (e.ClickedItem.Text.Trim() == "删除行")
            {
                cmsDg.Hide();
                DeleteRow();
            }
        ...
        public void InitDate()
        {
            ...
            btnStates(0);
            //派工类型
            txtSporadicType.Items.AddRange(new object[] { "计时派工", "零星机械" });
            txtSporadicType.SelectedItem = "计时派工";
        }
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                ...
                curBillMaster.LaborState = type;
                curBillMaster.CreateDate = dtpBusinessDate.Value.Date;
                curBillMaster.IsPlan = false;//逐日派工是计划数据，计时用工是实际数据
        ...
        void dgDetail_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (this.dgDetail.EditMode == DataGridViewEditMode.EditOnEnter)
                {
                    ...
                    else if (this.dgDetail.Columns[e.ColumnIndex].Name.Equals(colQuantityUnit.Name))
                    {
                        #region
                        //计时派工的单位固定为“工日”，不可改
                        if ("计时派工" == type)
                    ...
                    else if (this.dgDetail.Columns[e.ColumnIndex].Name.Equals(colLaborSubject.Name))
                    {
                        #region
                        //双击用工科目
                    ...
                    else if (this.dgDetail.Columns[e.ColumnIndex].Name.Equals(colLaborDescript.Name))
                    {
                        VTextSetDlg dlg = new VTextSetDlg(colLaborDescript.HeaderText);
                        dlg.StrInput = ClientUtil.ToString(this.dgDetail.CurrentRow.Cells[this.dgDetail.Columns[e.ColumnIndex].Name].Value);
                        dlg.ShowDialog();
                        this.dgDetail.Rows[e.RowIndex].Cells[colLaborDescript.Name].Value = dlg.StrInput;
                        this.dgDetail.Rows[e.RowIndex].Cells[colLaborDescript.Name].ReadOnly = true;
                        this.dgDetail.Rows[e.RowIndex].Cells[colLaborDescript.Name].ReadOnly = false;
                    }
                }
            }
            catch (Exception err)
        ...
        /// <returns></returns>
        private StandardUnit GetUnitByName(string strUnit)
        {
        ...
        private void DeleteRow()
        {
            ...
            {
                MessageBox.Show("最后一行空行不允许删除");
                return;
            }
            if (MessageBox.Show("确定要删除当前选中的记录吗？", "删除记录", MessageBoxButtons.YesNo,
