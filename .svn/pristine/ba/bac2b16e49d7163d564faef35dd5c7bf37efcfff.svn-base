...
using Application.Resource.PersonAndOrganization.SupplierManagement.Domain;
...
{
    public partial class VLaborSporadicMng : TMasterDetailView
    {
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (tabControl1.SelectedTab.Name.Equals(tabPage2.Name))
            {
                dgDocumentMast.Rows.Clear();
                dgDocumentDetail.Rows.Clear();
                if (curBillMaster != null && curBillMaster.Id != null&& dgDocumentMast.SelectedRows.Count>0)
                {
                    dgDocumentMast_CellClick(dgDocumentMast, new DataGridViewCellEventArgs(2, dgDocumentMast.SelectedRows[0].Index));
                }
            }
        ...
        void dgDetail_MouseClick(object sender, MouseEventArgs e)
        {
            if (!dgDetail.ReadOnly)
            {
                if (e.Button == MouseButtons.Right)
                {
                    if (dgDetail.Enabled)
                    {
                        cmsDg.Items[tsmiCopy.Name].Enabled = true;
        ...
        void tsmiDel_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确定要删除当前选中的记录吗？", "删除记录", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                ...
                if (dr.Tag != null)
                {
                    curBillMaster.Details.Remove(dr.Tag as LaborSporadicDetail);
                }
            }
        }
        ...
        public override void RefreshControls(MainViewState state)
        {
            ...
            //永久锁定
            object[] os = new object[] { txtSupply, txtCode, txtCreatePerson, txtHandlePerson, txtPredictProjectTast, txtRealProjectTast, txtProject };
            ObjectLock.Lock(os);
            string[] lockCols = new string[] { colProjectTastDetail.Name, colMaterialCode.Name, colProjectTastType.Name, colQuantityUnit.Name, colLaborSubject.Name, colInsteadTeam.Name};
        ...
        private bool ValidView()
        {
            ...
            foreach (DataGridViewRow dr in dgDetail.Rows)
            {
                ...
                if (dr.Cells[colInsteadTeam.Name].Value == null)
                {
                    MessageBox.Show("被代工队伍不能为空！");
                    dgDetail.CurrentCell = dr.Cells[colInsteadTeam.Name];
                    return false;
                }
                if ((dr.Cells[colInsteadTeam.Name].Tag as SubContractProject).Id == (txtSupply.Tag as SubContractProject).Id)
                {
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                ...
                curBillMaster.BearTeam = this.txtSupply.Tag as SubContractProject;
                ...
                foreach (DataGridViewRow var in this.dgDetail.Rows)
                {
                    ...
                    curBillDtl.InsteadTeam = var.Cells[colInsteadTeam.Name].Tag as SubContractProject;
                    ...
                    curBillDtl.RealLaborNum = ClientUtil.ToDecimal(var.Cells[colRealLaborNum.Name].Value);//实际用工量
                    curBillDtl.ResourceTypeName = ClientUtil.ToString(var.Cells[colLaborSubject.Name].Value);//资源类型
                    ...
                    curBillDtl.ProjectTastDetail = var.Cells[colProjectTastType.Name].Tag as GWBSDetail;//工程任务明细
                    ...
                    curBillDtl.ResourceTypeName = ClientUtil.ToString(var.Cells[colMaterialCode.Name].Value);
                    ...
                    //curBillDtl.SettlementState = EnumSettlementType.未结算;
                    curBillDtl.IsCreate = 0;
            ...
            if (this.dgDetail.EditMode == DataGridViewEditMode.EditOnEnter)
            {
                ...
                if (this.dgDetail.Columns[e.ColumnIndex].Name.Equals(colProjectTastDetail.Name))
                {
                    ...
                    if (frm.IsOk)
                    {
                        ...
                        //Application.Resource.MaterialResource.Domain.Material theMaterial = model.LaborSporadicSrv.SearchMaterial(strName);
                        Application.Resource.MaterialResource.Domain.Material theMaterial = null;
                        ObjectQuery oq = new ObjectQuery();
                        oq.AddCriterion(Expression.Eq("Name", strName));
                        IList lst = model.LaborSporadicSrv.GetDomainByCondition(typeof(Application.Resource.MaterialResource.Domain.Material), oq);
