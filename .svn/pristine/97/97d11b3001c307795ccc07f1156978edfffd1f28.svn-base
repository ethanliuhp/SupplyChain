...
{
    public partial class VMaterialMonthlyBalanceQuery : TBasicDataView
    {
        MMatRentalMng model = new MMatRentalMng();
        CurrentProjectInfo projectInfo = StaticMethod.GetProjectInfo();
        MaterialBalanceMaster currMaster = new MaterialBalanceMaster();
        private string rentalExp = "租赁费用";
        private string otherExp = "其他费用";

        public VMaterialMonthlyBalanceQuery()
        {
            InitializeComponent();
            InitData();
            InitEvent();
        ...
        void dgMaster_SelectionChanged(object sender, EventArgs e)
        {
            ...
            MaterialBalanceMaster master = dgMaster.CurrentRow.Tag as MaterialBalanceMaster;
            currMaster = master;
            if (master == null) return;
            //租赁费用
            foreach (MaterialBalanceDetail BalanceDetail in master.Details)
            {
                ...
                {
                    MaterialBalanceDetail t_BalDetail = new MaterialBalanceDetail();
                ...
                else
                {
                    ...
                    {
                        MaterialBalanceDetail Detail = list_temp[i] as MaterialBalanceDetail;
            ...
            //添加汇总信息
            foreach (MaterialBalanceDetail BalanceDetail in list_temp)
            {
                int rowIndex = dgCostDetail1.Rows.Add();
                dgCostDetail1[colMatCode.Name, rowIndex].Value = BalanceDetail.MaterialCode;
            ...
            //其他费用
            foreach (MatBalOtherCostDetail MatBalOtherCostDetail in master.MatBalOtherCostDetails)
            {
        ...
        void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                IList list = model.MatMngSrv.GetMatBalanceMaster(oq);
                ShowMasterList(list);
            }
            catch (Exception ex)
            {
