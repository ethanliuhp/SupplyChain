<%@ Control Language="C#" AutoEventWireup="true" CodeFile="GridViewDataMng.ascx.cs" Inherits="UserControls_GridViewDataMng" %>
<script src="<%=ResolveUrl("../JavaScript/Util.js")%>" type="text/javascript"></script>
 
 <style  type="text/css">
  ._SelectRowStyle{  color:#116a79 !important; background-color:#cef97b !important;  }
 ._MouseOverRowStyle{color:#116a79  !important; background-color:#cef97b  !important;}
 </style>
...
// }
 ...
 function _MouseOver(obj,gridID,iRowIndex){

    //var oGrid=document.getElementById(gridID);
//    if(oGrid && oGrid.selectRow && oGrid.selectRow.index==iRowIndex ){
//    }
//    else{
         // curBak=obj.style.backgroundColor;
          //curColor=obj.style.color;
         // obj.style.color='#116a79';
          //obj.style.backgroundColor='#cef97b';
    //}
    obj.className=obj.className+" _MouseOverRowStyle";
  }
 function _MouseOut(obj,gridID,iRowIndex){

     //var oGrid=document.getElementById(gridID);
//        if(oGrid && oGrid.selectRow && oGrid.selectRow.index==iRowIndex ){
//        }
//        else{
             //obj.style.backgroundColor=curBak;
             //obj.style.color=curColor;
       // }
       obj.className=obj.className.replace("_MouseOverRowStyle","");
 }
 function _Oprate(gridID,flag,iRowIndex,_fun,hdDeleteIDs){
    var oGrid=document.getElementById(gridID);
    _singleClick1(gridID,iRowIndex,_fun,hdDeleteIDs);
    if(oGrid){
    flag=toLower(flag);
     switch(flag){
            case "new":
            {    
                _executeFun(oGrid,_fun,flag);
                break;
            }
            case "delete":
            {
              if(oGrid.selectRow  ){
                 if( confirm('是否删除！')){
                     _deleteGridRow(oGrid,oGrid.selectRow.index,_fun,hdDeleteIDs);
                     _executeFun(oGrid,_fun,flag);
                     oGrid.selectRow=null;
                     alert("删除成功");
                   }
                }
                else{
 ...
 function _executeFun(oGrid,_fun,flag){
    if(_fun){
             if(oGrid.selectRow){
 ...
 function _singleClick1(gridID,iRowIndex,_fun,hdDeleteIDs){
     var oGrid=document.getElementById(gridID);
     if(oGrid && iRowIndex>0 && iRowIndex>0 && oGrid.rows.length>iRowIndex ){
        if(!oGrid.deleteRow){//没有删除属性
            oGrid.deleteRow=function(rowIndex){
                _deleteGridRow(oGrid,rowIndex,_fun,hdDeleteIDs);
            }
        }
  ...
  function _setSelectStyle1(oGrid,oTr){
    ...
    if(oGrid.tableLayout!='fixed' ){
        oGrid.tableLayout='fixed'
        for(var i=0 ;i< oTr.cells.length;i++){
            oTr.cells[i].noWrap=true;
        }
    }
    //oTr.style.color='#116a79';oTr.style.backgroundColor='#cef97b'
 }
 function _deleteGridRow(oGrid,iRowIndex,_fun,hdDeleteIDs){       
     if( oGrid.rows.length >iRowIndex){
         // oGrid.rows[iRowIndex].style.display='none';
           var oDeleteDetailIds=document.getElementById(hdDeleteIDs); 
           var sIDs=oDeleteDetailIds.value; 
           var arrIDs=[];
           if(sIDs){
                arrIDs=sIDs.split(';'); 
          ...
          if(_GetGridDisplayRowCount(oGrid)>1){
               oGrid.rows[iRowIndex].style.display='none';
              // debugger;
               _SetNum(oGrid);
            //oGrid.deleteRow(iRowIndex);
          }
          else{
              var isNew=true;//document.getElementById('<%=hdEnabledClientNewClick.ClientID %>').value=="True";
              if( isNew){
                  var oTr=oGrid.rows[iRowIndex];
                  for(var i=oTr.cells.length-1;i>0;i--){
                        oTr.deleteCell(i);
                   }
                   var newTD= oTr.cells[0];
                   newTD.colSpan=oGrid.rows[0].cells.length;
                   //hdEnabledClientNewClick  hdClientNewClickHTML  
                   var addImgUrl='<%=this.DestControl.Page.ResolveUrl("~/images/grid/add1.png") %>';
                   var sGridID=oGrid.id;
                   
                   //(gridID,flag,iRowIndex,_fun)
                    var img=document.createElement("img")
                    img.style.cursor="pointer";
                    img.src=addImgUrl;
                    img.onclick=function(){return _Oprate(sGridID,'new',-1,_fun,hdDeleteIDs);};
                    newTD.innerHTML="";
 ...
 function _GetGridDisplayRowCount(oGrid){
    var iRowCount=0;
    if(oGrid){
 ...
 function _SetNum(oGrid){
    if(oGrid && _GetGridDisplayRowCount(oGrid)>0 && oGrid.rows[1].cells.length>1 ){
        var iColumnIndex=-1;
        var iRowNum=1;
        for(var i=0;i<oGrid.rows[0].cells.length;i++){
            if(oGrid.rows[0].cells[i].innerText=='序号'){
                iColumnIndex=i;
                break;
            }
        }
        if(iColumnIndex>-1){
            for(var i=1;i<oGrid.rows.length;i++){
                if(oGrid.rows[i].style.display!="none"){
                    oGrid.rows[i].cells[iColumnIndex].innerText=iRowNum;
                    iRowNum++;
                }
            }
        }
...
<div id="divPager" style="height: 22px;width: 99%;  border:solid 0px blue; top: 0px; margin: 0px 0px 0px 0px; padding: 8px 0px 0px 0px; <%=this.RowCount==0?"display:none;": "" %>   " align="right">
    <table cellpadding="0" cellspacing="0"  style="width:auto;white-space: nowrap; border:solid 0px blue; ">
            ...
            <tr >
                ...
                <td>
                    跳到
                    <asp:DropDownList ID="selPager" runat="server" Style="width: 40px; padding:0px; margin:0px;" AutoPostBack="true"
...
<div style="display: none">
    ...
    <asp:HiddenField ID="hdClientNewClickHTML" Value=""  runat="server"/>
