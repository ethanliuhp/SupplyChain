...
{
    public partial class VGWBSConfirmBak2 : TMasterDetailView
    {
        ...
        void dgDetail_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            GWBSDetail detail = dgDetail.Rows[e.RowIndex].Cells[colGWBSDetail.Name].Tag as GWBSDetail;

            gridFinishedTask.Rows.Clear();
            if (dicDetail.Count > 0 && dicDetail.ContainsKey(detail.Id))
            {
                foreach (WeekScheduleDetail dtl in dicDetail[detail.Id])
                {
                    int index = gridFinishedTask.Rows.Add();
                    DataGridViewRow row = gridFinishedTask.Rows[index];
                    row.Cells[colFinishTask.Name].Value = dtl.GWBSTree.Name;
                    row.Cells[colFinishTask.Name].ToolTipText = StaticMethod.GetCategorTreeFullPath(typeof(GWBSTree), dtl.GWBSTree);
                    row.Cells[colFinishAddupFigureprogress.Name].Value = dtl.TaskCompletedPercent;
        ...
        void btnProductSchedule_Click(object sender, EventArgs e)
        {
            ...
            List<GWBSDetail> listSelTaskDtl = new List<GWBSDetail>();

            foreach (WeekScheduleDetail WeekDtl in listWeekDtl)
            {
                //从周计划所在的任务节点往上查（生产）任务明细

                string sysCode = WeekDtl.GWBSTree.SysCode;
                string[] sysCodes = sysCode.Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries);

                for (int i = sysCodes.Length - 2; i > -1; i--)//从父节点开始一直往上查
                {
                    string parentSysCode = "";
                    for (int j = i; j > -1; j--)
                    {
                        parentSysCode = sysCodes[j] + "." + parentSysCode;
                    }

                    oq.Criterions.Clear();
                    oq.FetchModes.Clear();
                    oq.AddCriterion(Expression.Eq("ProduceConfirmFlag", 1));
                    oq.AddCriterion(Expression.Eq("TheGWBS.SysCode", parentSysCode));
                    ...
                    if (list.Count > 0)//查到了生产明细就不再往上查
                    {
                        foreach (GWBSDetail taskDtl in list)
                        {
                            if (!dicDetail.Keys.Contains(taskDtl.Id))
                            {
                                dicDetail.Add(taskDtl.Id, new List<WeekScheduleDetail>() { WeekDtl });

                                listSelTaskDtl.Add(taskDtl);
                            }
                            else
                            {
            ...
            //dgDetail.Rows.Clear();
            foreach (GWBSDetail detail in listSelTaskDtl)
            {
                if (HasAdded(detail))
                    continue;
                ...
                List<WeekScheduleDetail> listWeek = dicDetail[detail.Id];

                taskConfirmDetail.WeekScheduleDetail = listWeek;

                if (listWeek.Count == 1 && listWeekDtl[0].SubContractProject != null)
                {
                    taskConfirmDetail.TaskHandler = model.ProductionManagementSrv.GetObjectById(typeof(SubContractProject), listWeekDtl[0].SubContractProject.Id) as SubContractProject;
                }
            ...
            if (dgDetail.CurrentRow != null)
            {
                ...
                GWBSDetail detail = dgDetail.CurrentRow.Cells[colGWBSDetail.Name].Tag as GWBSDetail;
                foreach (WeekScheduleDetail dtl in dicDetail[detail.Id])
                {
                    int index = gridFinishedTask.Rows.Add();
                    DataGridViewRow row = gridFinishedTask.Rows[index];
                    ...
                    row.Cells[colFinishTask.Name].Value = dtl.GWBSTree.Name;
                    row.Cells[colFinishTask.Name].ToolTipText = StaticMethod.GetCategorTreeFullPath(typeof(GWBSTree), dtl.GWBSTree.Name, dtl.GWBSTree.SysCode);
                    row.Cells[colFinishAddupFigureprogress.Name].Value = dtl.TaskCompletedPercent;
