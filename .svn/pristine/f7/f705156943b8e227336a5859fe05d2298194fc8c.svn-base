...
{
    public partial class VProjectBusinessStateMng : TBasicDataView
    {
        ...
        public VProjectBusinessStateMng(CProjectDepartment_ExecType execType)
        {
            InitializeComponent();
        ...
        private void InitDate()
        {
            ...
            {
                //DataGridViewTextBoxColumn oProjectCurrState = new DataGridViewTextBoxColumn()
                //{
                ...
                // dgDetail.Columns.Insert(ColProjectType.Index + 1, oProjectCurrState);
                ...
                // oProjectCurrState.DefaultCellStyle.BackColor = oProjectinfoState.DefaultCellStyle.BackColor = System.Drawing.SystemColors.Control;
                lblName.Text = "工程状态";
            ...
            else if (execType == CProjectDepartment_ExecType.ProjectBusinessStateMng)
            {
                ...
                //DataGridViewTextBoxColumn oProjectinfoState = new DataGridViewTextBoxColumn()
                //{
                //    Name = "colProjectinfoState",
                //    HeaderText = "工程状态",
                //    Width = 100,
                //    ReadOnly = true
                //};
                //dgDetail.Columns.Insert(ColProjectType.Index + 2, oProjectinfoState);
                //oProjectCurrState.DefaultCellStyle.BackColor = oProjectinfoState.DefaultCellStyle.BackColor = System.Drawing.SystemColors.Control;
                lblName.Text = "商务状态";
            ...
            if (oCmbColumn != null)
            {
                ...
                //oCmbColumn.DefaultCellStyle.ForeColor = System.Drawing.Color.White; 
                oCmbColumn.DisplayStyle = DataGridViewComboBoxDisplayStyle.ComboBox;
                oCmbColumn.FlatStyle = FlatStyle.Popup;
                // oCmbColumn.DefaultCellStyle.SelectionBackColor = System.Drawing.Color.White;
                // oCmbColumn.DefaultCellStyle.SelectionForeColor = System.Drawing.Color.White; 
            }
            QueryProjectInfo();
        }
        ...
        private void QueryProjectInfo()
        {
            ...
            //    oBuilder.AppendFormat(" and t.ConstractStage='{0}' ", projectStage);
            //}
            try
            {
        ...
        private void ShowProject(DataSet dsProjectInfos)
        {
            ...
            foreach (DataRow oRow in oTable.Rows)
            {
                ...
                dr.Cells[RankingNote.Name].Value = oRow["projecttypedescript"] + "";
            }
        ...
        public void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (CProjectDepartment_ExecType.ProjectBusinessStateMng == execType)
                {
                    sSQLTemp = " update resconfig set projectcurrstate={0},projecttypedescript='{2}'  where id='{1}'";
                    foreach (DataGridViewRow oRow in dgDetail.Rows)
                    {
                        if (oRow.Tag != null)
                        {
                            oDataRow = oRow.Tag as DataRow;
                            iProjectCurrState = (int)Enum.Parse(typeof(EnumProjectCurrState), ClientUtil.ToString(oRow.Cells["colProjectCurrState"].Value));

                            var projectTypeNote = oRow.Cells[RankingNote.Name].Value + "";

                            if (iProjectCurrState != ClientUtil.ToInt(oDataRow["projectcurrstate"]) || projectTypeNote != oDataRow["projecttypedescript"] + "")
                            {
                                sID = ClientUtil.ToString(oDataRow["id"]);
                                if (!string.IsNullOrEmpty(sID))
                                {
                                    lstCurProjectSQL.Add(string.Format(sSQLTemp, iProjectCurrState, sID, projectTypeNote));
                                }
                            }
                        }
                    }
