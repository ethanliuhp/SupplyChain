    ...
    public partial class VSelFeeTemplate : TMasterDetailView
    {
        ...
        void menuItemDeleteRow_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实要删除选择的行吗？", "删除提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                {
                    foreach (DataGridViewRow row in this.dgDetail.SelectedRows)
                    {
                        if (row.IsNewRow)
                            break; 

                        SelFeeTemplateDtl item = row.Tag as SelFeeTemplateDtl;
                        if (item != null)
                        {
                            curBillMaster.SelFeeTemplateDetails.Remove(item);
                            this.dgDetail.Rows.Remove(row);
                        }
                    }
                }
                else
                {
                    foreach (DataGridViewRow row in this.dgRules.SelectedRows)
                    {
                        if (row.IsNewRow)
                            break;
                        SelFeeTempFormula item = row.Tag as SelFeeTempFormula;
                        if (item != null)
                        {
                            curBillMaster.SelFeeTempFormulas.Remove(item);
                            this.dgRules.Rows.Remove(row);
                        }
                    }
        ...
        private void contextMenuStrip1_Opening(object sender, CancelEventArgs e)
        {
            ...
            this.menuItemDeleteRow.Enabled = isEdit && ((this.tabControl1.SelectedTab==this.tabPage1 && this.dgDetail.Rows.Count>1) || (this.tabControl1.SelectedTab== this.tabPage2 && this.dgRules.Rows.Count>1));
        }
