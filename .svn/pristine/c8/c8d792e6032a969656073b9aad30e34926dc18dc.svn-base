...
{
    public partial class VLaborSporadicPlan : TMasterDetailView
    {
        ...
        public VLaborSporadicPlan()
        {
            ...
            InitDate();
            InitSporadicType();
            this.txtSporadicType.Text = "逐日派工";
            //colPredictLaborNum.HeaderText = "计划工程量";
        }

        private void InitSporadicType()
        {
            //派工类型
            txtSporadicType.Items.AddRange(new object[] { "计时派工", "逐日派工" });
        }
        ...
        {
            //if (txtSporadicType.SelectedItem.Equals("逐日派工"))
            //{
            //    colPredictLaborNum.HeaderText = "计划用工量";
            //}
        ...
        public override void RefreshControls(MainViewState state)
        {
            ...
            ObjectLock.Lock(os);
            string[] lockCols = new string[] { colProjectTastDetail.Name, colProjectTastType.Name, colMaterialCode.Name, colQuantityUnit.Name, colLaborSubject.Name };
        ...
        private bool ValidView()
        {
            ...
            {
                if (ClientUtil.ToString(dr.Cells[colProjectTastType.Name].Value) != "" && dr.Cells[colPredictLaborNum.Name].Value == null)
                {
                    MessageBox.Show("计划用工量不能为空！");
                    return false;
                }
                //最后一行不进行校验
                ...
                if (dr.Cells[colPredictLaborNum.Name].Value == null)
                {
                    MessageBox.Show("计划用工量不能为空！");
                    dgDetail.CurrentCell = dr.Cells[colPredictLaborNum.Name];
                    return false;
                }
            }
            return true;
        ...
        private bool ViewToModel()
        {
            ...
            try
            {
                ...
                curBillMaster.IsPlan = true;//逐日派工是计划数据，零星用工是实际数据
                ...
                foreach (DataGridViewRow var in this.dgDetail.Rows)
                {
                    ...
                    curBillDtl.PredictLaborNum = ClientUtil.ToDecimal(var.Cells[colPredictLaborNum.Name].Value);//计划用工量
                    curBillDtl.EndDate = Convert.ToDateTime(var.Cells[colEndDate.Name].Value).Date;//结束时间
                    ...
                    curBillDtl.StartDate = Convert.ToDateTime(var.Cells[colStartDate.Name].Value).Date;//起始时间
                    ...
                    curBillDtl.WorkersInformation = ClientUtil.ToString(var.Cells[colWorkersInformation.Name].Value);//人员配置信息
        ...
        private void dgDetail_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (colName == colPredictLaborNum.Name)//计划工程量
                {
                    string quantity = ClientUtil.ToString(dgDetail.Rows[i].Cells[colPredictLaborNum.Name].Value);
                    if (quantity == null)
                    {
                        quantity = "0";
                        return;
                    ...
                    if (validity == false)
                    {
                        MessageBox.Show("计划用工量为数字！");
                        this.dgDetail.Rows[i].Cells[colPredictLaborNum.Name].Value = "";
                        flag = false;
                    }
