...
strFrame+='<td width=20 height=20 id=meizzDay'+n+' style="font-size:12px;color:#900;" Author=meizz onclick=parent.meizzDayClick(this.innerText,0)></td>';n++;}
strFrame+='</tr>';}
...
function setday(tt,obj) //主调函数
{
 ...
 dads.left = tleft;
 dads.width=148;
 dads.height=211;//modify by sloppy
...
function meizzToday()  //Today Button
 ...
 var today;
    ...
    //meizzSetDay(meizzTheYear,meizzTheMonth);
    if(outObject){
    outObject.value=meizzTheYear + "-" + meizzTheMonth + "-" + today + " " + hour + ":"+ minute + ":" + second;
    }
    closeLayer();
  }
...
function meizzSetDay(yy,mm)   //主要的写程序**********
{
  ...
  else  //本月的部分
  {
   ...
   else da.innerHTML="<b>" + meizzWDay[i] + "</b>";
 ...
 if(mm<1){
  ...
  if (outObject)
  {
    if (!n) {//outObject.value=""; 
      return;}
    if ( n < 10){n = "0" + n;}
    outObject.value= yy + "-" + mm + "-" + n + " 00:00:00" ; //注：在这里你可以输出改成你想要的格式
    outObject.fireEvent('onchange');   //触发一个onChange事件 
    closeLayer(); 
