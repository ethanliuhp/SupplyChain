...
namespace Application.Business.Erp.SupplyChain.Client.CostManagement.CostMonthAccount
{
    public partial class VCostMonthAccount : TBasicDataView
    {
        ...
        private void ControlTextDisplayInfo()
        {
            if (ProjectInfo.ProjectInfoState == EnumProjectInfoState.老项目)
            {
                this.lbConstructionArea.Visible = false;
                this.txtConstructionArea.Visible = false;
                return;
            }
            string syear = this.cmbYear.Text;
            string smonth = this.cboFiscalMonth.Text;
            int int_month = ClientUtil.ToInt(smonth);
            if (int_month < 10) smonth = "0" + smonth;
            this.lbConstructionArea.Text = syear + "年" + smonth + "月累计建筑面积：";
            this.txtConstructionArea.Text = GetConstructionArea();
        
        }

        private string GetConstructionArea()
        {
            ap = null;
            ObjectQuery oq = new ObjectQuery();
            oq.AddCriterion(Expression.Eq("ProjectId", ProjectInfo.Id));
            oq.AddCriterion(Expression.Eq("Year",ClientUtil.ToInt(this.cmbYear.Text)));
            oq.AddCriterion(Expression.Eq("Month",ClientUtil.ToInt(this.cboFiscalMonth.Text)));
            IList lst = model.CommonMethodSrv.Query(typeof(SumAreaParame),oq);
            if (lst != null && lst.Count > 0)
            {
                ap = lst[0] as SumAreaParame;
                return ClientUtil.ToString(ap.ConstructionArea);
            }
            return "";
        }
        ...
        void btnReckoning_Click(object sender, EventArgs e)
        {
            ...
            if (ProjectInfo != null && ProjectInfo.ProjectInfoState == EnumProjectInfoState.新项目)
            {
                try
                {
                    Convert.ToDecimal(this.txtConstructionArea.Text);
                }
                catch
                {
                    MessageBox.Show("【累计建筑面积】只能为数字！");
                    this.txtConstructionArea.Focus();
                    return;
                }
                if (ap == null)
                {
                    ap = new SumAreaParame();
                    ap.ProjectId = ProjectInfo.Id;
                    ap.Year = ClientUtil.ToInt(this.cmbYear.Text);
                    ap.Month = ClientUtil.ToInt(this.cboFiscalMonth.Text);
                    ap.Resconfig = ProjectInfo;
                    ap.ConstructionArea = ClientUtil.ToDecimal(this.txtConstructionArea.Text);
                }
                else
                {
                    ap.ConstructionArea = ClientUtil.ToDecimal(this.txtConstructionArea.Text);
                ...
                ap = model.CostMonthAccSrv.SaveSumAreaParame(ap);
            }
            ...
            if (ProjectInfo != null && ProjectInfo.ProjectInfoState == EnumProjectInfoState.新项目)
            {
                bill.TempCalculateArea = ClientUtil.ToDecimal(this.txtConstructionArea.Text);
                bill.TempProjectArea = ProjectInfo.BuildingArea;
                bill.TempVatRate = ProjectInfo.VatRate;
                bill.TempMeasuresFeeRatio = ProjectInfo.MeasuresFeeRatio;
                bill.TempFeesRatio = ProjectInfo.FeesRatio;
                bill.TempManagementFeeRatio = ProjectInfo.ManagementFeeRatio;
            }
