...
{
    public partial class VExpensesSingleSearchCon : BasicUserControl
    {
        private MExpensesSingleBillMng model = new MExpensesSingleBillMng();
        private VExpensesSingleSearchList searchList;

        public VExpensesSingleSearchCon(VExpensesSingleSearchList searchList)
        {
        ...
        void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (this.txtExpensesPerson.Text != "")
            {
                oq.AddCriterion(Expression.Eq("ExpensesPerson", this.txtExpensesPerson.Text));
            }
            oq.AddCriterion(Expression.Ge("ExpensesDate", this.dtpDateBegin.Value.Date));
            oq.AddCriterion(Expression.Lt("ExpensesDate", this.dtpDateEnd.Value.AddDays(1).Date));
            oq.AddOrder(new Order("Code", false));
            IList list = model.ExpensesSingleBillSrv.GetExpensesSingleBill(oq);
