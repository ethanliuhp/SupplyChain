...
namespace Application.Business.Erp.SupplyChain.Client.MoneyManage
{
    public partial class VFundSchemeRequest : TMasterDetailView
    {
        ...
        private DataTable projectPersons;
        ...
        private void AfterSelectProject(object sender)
        {
            ...
            if (sp.SelectedProject != null)
            {
                ...
                var ds = mOperate.CurrentProjectSrv.LoadPerson(sp.SelectedProject.OwnerOrg.Id);
                if (ds != null && ds.Tables.Count > 0)
                {
                    projectPersons = ds.Tables[0];
                }
            }
            else
            {
        ...
        private bool ModelToView()
        {
            try
            {
                ...
                ucAttachment1.FillDoc(selectFund);
        ...
        private bool ValidView()
        {
            ...
            decimal tmp = 0;
            if (!decimal.TryParse(txtProfitRate.Text.Trim(), out tmp) || tmp < -100 || tmp > 100)
            {
                txtProfitRate.Focus();
            ...
            if (!decimal.TryParse(txtHandIn.Text.Trim(), out tmp) || tmp < -100 || tmp > 100)
            {
                txtHandIn.Focus();
            ...
            if (!decimal.TryParse(txtPayRate.Text.Trim(), out tmp) || tmp < -100 || tmp > 100)
            {
                txtPayRate.Focus();
            ...
            if (!decimal.TryParse(txtPaymentBalance.Text.Trim(), out tmp) || tmp < -100 || tmp > 100)
            {
        ...
        private void LoadFundScheme()
        {
            ...
            ucAttachment1.CurrentBill = selectFund;
            ucAttachment1.ClearDoc();
        }
        ...
        private void NewFundScheme()
        {
            ...
            if (projectPersons != null && projectPersons.Rows.Count > 0)
            {
                var rows = projectPersons.Select(" opjname = '项目财务' or opjname = '成本员' ").OrderBy(a => a.Field<string>("pername"));
                if (rows.Any())
                {
                    selectFund.FinancePerson = rows.First()["pername"].ToString();
                }

                rows = projectPersons.Select(" opjname = '项目商务经理' or opjname = '预算员' ").OrderBy(a => a.Field<string>("pername"));
                if (rows.Any())
                {
                    selectFund.BusinessPerson = rows.First()["pername"].ToString();
                }

                rows = projectPersons.Select(" opjname = '项目总工程师' or opjname = '技术员' ").OrderBy(a => a.Field<string>("pername"));
                if (rows.Any())
                {
                    selectFund.TechnologyPerson = rows.First()["pername"].ToString();
                }

                rows = projectPersons.Select(" opjname = '项目经理' ").OrderBy(a => a.Field<string>("pername"));
                if (rows.Any())
                {
                    selectFund.DirectorPerson = rows.First()["pername"].ToString();
                }
            }
        ...
        private bool SaveFundScheme(bool isSubmit)
        {
            try
            {
                ...
                selectFund.DocState = isSubmit ? DocumentState.InAudit : DocumentState.Edit;
                ...
                MessageBox.Show((isSubmit ? "提交" : "保存") + "成功！");
        ...
        public override void RefreshControls(MainViewState state)
        {
            ...
            if (ViewState == MainViewState.AddNew || ViewState == MainViewState.Modify)
            {
                ...
                if (selectFund != null && ViewState == MainViewState.Modify)
                {
                    var details =
                        mOperate.FinanceMultDataSrv.GetFundSchemeDetail(selectFund, typeof(FundSchemeReportAmount));
                    if (details != null && details.Count > 0)
                    {
                        ObjectLock.Lock(new object[] {dtpBeginDate, dtpEndDate});
                    }
                }
            }
            else
        ...
        public override bool DeleteView()
        {
            ...
            if (MessageBox.Show("删除后将无法恢复，您确定删除该记录？", "删除确认", MessageBoxButtons.YesNo, MessageBoxIcon.Question) !=
                DialogResult.Yes)
            {
                return false;
            }
        ...
        {
            if (string.IsNullOrEmpty(selectFund.Id) || selectFund.DocState != DocumentState.Edit)
            {
                MessageBox.Show("请选择编辑中的策划表提交审核");
                return false;
            }
            ...
            var amountList = mOperate.FinanceMultDataSrv.GetFundSchemeDetail(selectFund, typeof (FundSchemeReportAmount));
            if (amountList == null || amountList.Count == 0)
            {
                MessageBox.Show("该策划表没有测算数据，请进入【资金测算表】编制数据再提交审核");
                return false;
            }

            if(MessageBox.Show("您确认要提交审核该策划表？","提交确认",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.No)
            {
                return false;
            }
